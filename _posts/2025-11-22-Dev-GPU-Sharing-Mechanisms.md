...
위 메커니즘들은 GPU 드라이버·하드웨어 수준의 기능이지만, 쿠버네티스 환경에서 이를 적용할 때 한 가지 중요한 점이 있다. **"분수 GPU"는 request/limit burst가 아니라, Device Plugin이 광고하는 단위 자체를 바꾸는 별개 메커니즘**이라는 것이다.

`nvidia.com/gpu`는 쿠버네티스의 [Extended Resource]({% post_url 2024-07-23-Dev-Kubernetes-NVIDIA-GPU-Mechanism %}#extended-resource로서의-gpu)이므로 정수만 허용되고, request == limit이 강제되며, overcommit이 불가능하다. 이 규칙은 GPU 공유 메커니즘을 적용해도 **바뀌지 않는다.** 바뀌는 건 "한 장치를 몇 개로 보여줄 것인가"다.

| 방식 | Device Plugin이 광고하는 것 | Pod 요청 예시 | 실체 |
|------|----------------------------|---------------|------|
| Time Slicing | `nvidia.com/gpu`를 replicas 수만큼 | `nvidia.com/gpu: 1` | 물리 GPU 1개를 컨텍스트 스위칭으로 공유. 격리 없음 |
| MPS | `nvidia.com/gpu`를 replicas 수만큼 | `nvidia.com/gpu: 1` | MPS 데몬을 통한 프로세스 동시 실행 + 공간 분할. 메모리·컴퓨트를 replica 수로 균등 파티셔닝 |
| MIG | `nvidia.com/mig-1g.5gb` 등 **별도 리소스 타입** | `nvidia.com/mig-1g.5gb: 1` | 하드웨어 물리 분할. 아예 다른 리소스 이름으로 광고 |

세 방식 모두 Pod 매니페스트에서 쓰는 request/limit 의미론(정수, request == limit)은 그대로다. Time Slicing과 MPS는 물리 GPU 1개를 `replicas: N`으로 뻥튀기하여 논리 GPU N개로 [광고]({% post_url 2025-11-22-Kubernetes-GPU-Time-Slicing-2 %}#1단계-보고-listandwatch)하고, MIG는 하드웨어 분할된 인스턴스를 `nvidia.com/mig-*` 형태의 별도 리소스 타입으로 광고한다. 어느 쪽이든 Pod은 여전히 `nvidia.com/gpu: 1` 또는 `nvidia.com/mig-1g.5gb: 1`처럼 정수 단위로 요청하며, 그 "1"이 가리키는 물리적 실체가 달라지는 것일 뿐이다.

> **참고: Time Slicing과 MPS는 상호 배타적이다**
>
> Device Plugin에서 `sharing` 설정은 `timeSlicing` 또는 `mps` 중 하나만 선택할 수 있다. 노드 레이블 `nvidia.com/gpu.sharing-strategy`도 `none`, `time-slicing`, `mps` 중 택일이다. 둘 다 `replicas: N`으로 광고한다는 공통점이 있지만, Time Slicing은 컨텍스트 스위칭으로 시분할하는 반면, MPS는 MPS 데몬을 통해 프로세스를 동시에 실행하면서 메모리와 컴퓨트를 `1/N`로 균등 파티셔닝한다. "Time Slicing으로 광고하고 MPS로 실행한다"는 레이어 구조가 아니라, Device Plugin이 sharing strategy에 따라 광고 방식과 실행 방식을 한 번에 결정하는 구조다.
