...
이 축에서 보면, GPU ECC는 메모리 컨트롤러가 매 접근마다 처리하는 **하드웨어 ECC** 쪽이다.
...
정리하면, ECC는 **"여분 정보로 비트 오류를 잡아내고(검출), 가능하면 되돌리는(정정) 부호화 기법"**이다. 메모리에서는 컨트롤러가 매 접근마다 실시간·자동으로, 통신·스토리지에서는 코드가 명시적으로 호출할 때 쓴다. 

이 축으로 보면, 이 글에서 다루는 GPU ECC는 **하드웨어 + 검출·정정** 사분면에 있다.

<br>

# 비트는 어떻게, 왜 뒤집히는가

여기서부터는 관심사를 **하드웨어 메모리 ECC**로 좁혀 보자. 앞에서 ECC가 여분 정보로 오류를 검출·정정하는 부호화 기법이라는 걸 봤는데, 그렇다면 메모리에서는 애초에 무슨 오류가 생기길래 ECC가 필요한 걸까? 메모리 ECC가 무엇을 막는지 이해하려면, 먼저 메모리에서 비트가 왜 뒤집히는지부터 알아야 한다.
...
그런데 패리티 비트가 어떻게 "어느 비트가 틀렸는지"까지 알아낼까? 핵심 직관은 **패리티 하나가 예/아니오 질문 하나**라는 것이다. 질문을 여러 개 겹쳐 던지면, 답의 조합이 틀린 비트의 좌표가 된다.

- **패리티 1개**: 데이터 비트들의 1의 개수가 짝수냐 홀수냐만 본다. 비트 하나가 뒤집히면 홀짝이 깨지니 "틀렸다"는 건 안다. 하지만 어느 자리가 뒤집혀도 답은 똑같이 "홀수"라, 어느 비트인지는 모른다 → **검출만** 가능
- **패리티 여러 개**: 패리티마다 데이터의 서로 다른 부분집합을 감시하게 배치한다(Hamming 부호). 비트 하나가 뒤집히면 그 비트를 감시하던 패리티들만 깨지고, **깨진 패리티들의 조합이 곧 틀린 비트의 위치**를 가리킨다. 이 조합을 **신드롬(syndrome)**이라 부른다. 위치를 알면 그 비트만 도로 뒤집어 복구 → **정정(SEC)**
- **전체를 덮는 패리티 1개 추가**: 데이터 전체를 한 번 더 덮는 패리티를 얹으면 뒤집힌 비트의 **개수**가 홀수인지 짝수인지를 알 수 있다. 단일 오류(1개, 홀수)면 이 패리티도 같이 깨지지만, 이중 오류(2개, 짝수)면 멀쩡해 보인다. 신드롬은 뭔가 났다는데 전체 패리티는 멀쩡하다 → "짝수 개가 뒤집혔다" → **검출(DED)**

여기서 **DED는 이름 그대로 검출까지만이고, 정정은 못 한다**는 점이 중요하다. 이중 오류에서 신드롬은 두 위치가 섞인 값이라 멀쩡한 제3의 위치를 가리키는데, 그 말을 믿고 고치면 틀린 비트가 오히려 하나 더 늘어난다. 하드웨어가 정정을 안 하는 게 아니라 못 하는 것이고, 그래서 정정을 포기하고 "고칠 수 없는 오염이 났다"고 신고만 한다.

정리하면, **패리티를 하나만 쓰면 검출, 여러 개를 겹쳐 위치를 특정하면 정정, 거기에 하나를 더 얹어 이중 오류까지 구분하는 게 SECDED**다. 패리티 배치와 신드롬 계산이 실제로 어떤 모양인지는 [부록: 패리티는 어떻게 틀린 비트의 위치를 짚는가](#부록-패리티는-어떻게-틀린-비트의-위치를-짚는가)에 도식으로 정리해 뒀다.
...
> 운영 관점에서 실제로 다루는 건 패리티 계산이 아니라 카운터다. 신드롬 역산과 정정은 하드웨어가 자동·투명하게 다 해주고, 우리가 보는 건 `nvidia-smi`의 `Correctable`(SEC가 조용히 고친 횟수)과 `Uncorrectable`(DED가 잡았지만 못 고친 것) 숫자뿐이다. 그래서 "단일은 고치고 이중은 잡아만 낸다"는 동작 모델까지만 알면 충분하다. 신드롬이 실제로 어떻게 계산되고 왜 이중 오류는 정정이 불가능한지가 궁금하다면 글 마지막의 [부록](#부록-패리티는-어떻게-틀린-비트의-위치를-짚는가)을 참고하자. 
...
# 부록: 패리티는 어떻게 틀린 비트의 위치를 짚는가

본문에서는 "패리티를 여러 개 겹치면 깨진 조합이 위치를 가리킨다"까지만 말하고 넘어갔다. 그 조합이 어떻게 위치가 되는지, 그리고 이중 오류에서는 왜 그 위치를 믿으면 안 되는지를 도식으로 풀어 둔다. 운영에서 이 계산을 손으로 할 일은 없지만, `Correctable`과 `Uncorrectable`이 갈리는 지점이 바로 여기다.

## 패리티 1개는 왜 위치를 모르나

데이터 `1 0 1 1 0 0 1 0`의 1의 개수는 4개, 짝수다. 짝수 패리티(even parity)를 쓴다면 패리티 비트로 `0`을 저장한다.

여기서 **어느 비트가 뒤집히든** 1의 개수는 홀수가 되고, 패리티는 똑같이 "불일치"를 외친다. 1번 자리가 뒤집혀도 불일치, 7번 자리가 뒤집혀도 불일치다. 패리티 하나가 내놓는 답은 깨졌다/안 깨졌다는 1비트뿐인데 구별해야 할 경우의 수는 여덟 가지이니, 정보량이 모자라서 위치를 못 짚는 것이다.

거꾸로 말하면, 여덟 자리 중 하나를 특정하려면 예/아니오 질문이 세 개 필요하다(`2^3 = 8`). **패리티 하나가 곧 질문 하나**이고, 질문을 겹쳐 던지면 답의 조합이 좌표가 된다. 정정의 원리는 이게 전부다.

## 격자로 보는 위치 특정

가장 눈에 잘 들어오는 형태는 2차원 격자다. 데이터 16비트를 4×4로 놓고 행마다 패리티 하나, 열마다 패리티 하나를 붙인다. 아래는 `r2c3` 비트가 뒤집힌 채로 읽어 온 상황이다.

```text
            c1  c2  c3  c4 │ 행 패리티
                           │ 저장   재계산   판정
    r1       1   0   1   0 │  0      0      일치
    r2       0   1   0   0 │  0      1       ✗    ← 이 행 어딘가
    r3       1   1   0   1 │  1      1      일치
    r4       0   0   1   1 │  0      0      일치
   ────────────────────────
    저장     0   0   1   0
...
깨진 행(`r2`)과 깨진 열(`c3`)의 **교차점**인 `r2c3`이 범인이다. 그 한 자리만 도로 뒤집으면 복구가 끝난다. 각 패리티는 "내 담당 구역 안에 오류가 있냐"에만 답하는데, 구역들이 서로 어긋나게 겹쳐 있어서 답의 조합이 좌표가 되는 것이다.

다만 이 방식은 16비트에 패리티가 8개(행 4 + 열 4)나 필요해서 비싸다.

## Hamming 부호: 좌표를 이진수로 찍기

Hamming 부호는 감시 구역을 행/열이 아니라 **위치 번호의 이진수 자릿수**로 나눈다. 패리티 `P1`은 위치 번호의 1의 자리가 1인 자리들을, `P2`는 2의 자리가 1인 자리들을 감시하는 식이다.

```text
위치           1     2     3     4     5     6     7
위치의 이진수  001   010   011   100   101   110   111
...
이제 5번 비트가 뒤집혔다고 하자. `5 = 101`이니 5를 감시하는 패리티는 `P1`과 `P4`이고, `P2`는 5를 보지 않는다.

```text
           P4    P2    P1
결과        ✗    일치    ✗
읽으면      1     0     1   →   binary 101 = 5   ← 뒤집힌 비트의 위치
```

우연이 아니라 그렇게 되도록 배치한 것이다. 감시 구역을 위치 번호의 자릿수로 잡았으니, **깨진 패리티들을 자릿수 순서로 읽으면 그 값이 곧 뒤집힌 위치의 이진 표현**이 된다. 이 값이 본문에서 말한 신드롬이고, `000`이면 오류가 없다는 뜻이다.

격자 방식과 비교하면 훨씬 싸다. 7자리를 패리티 3개로 덮고, 64비트 데이터라면 `2^7 = 128`이 `64 + 7 + 1 = 72` 이상이므로 패리티 7개로 위치를 다 표현할 수 있다. 여기에 아래에서 볼 DED용 1개를 더해 8개 → 앞에서 본 `(72,64)` 구성이 나온다.

## 이중 오류: 검출은 되고 정정은 안 되는 이유

두 비트가 동시에 뒤집히면 신드롬은 **두 위치를 XOR한 값**이 된다. 3번(`011`)과 6번(`110`)이 뒤집힌 경우를 따라가 보자.

```text
      3번(011) 감시?   6번(110) 감시?   깨진 횟수      결과
P1         O                X            1번(홀수)      ✗
P2         O                O            2번(짝수)     상쇄되어 일치
P4         X                O            1번(홀수)      ✗

신드롬 = (P4, P2, P1) = 1 0 1 = 5      ... 011 XOR 110 = 101
```

멀쩡한 **5번**을 틀렸다고 지목한다. 이 말을 믿고 5번을 뒤집으면 틀린 비트가 3·5·6 세 개로 늘어나, 고치기 전보다 나빠진다. **이중 오류에서 신드롬은 위치 정보로서 아무 의미가 없다.**

그래서 데이터와 패리티 전체를 한 번 더 덮는 패리티를 하나 얹는다. 얘는 위치는 전혀 모르고 **뒤집힌 비트의 개수가 홀수냐 짝수냐**만 안다. 이 하나가 SEC와 DED를 가른다.

| 신드롬 | 전체 패리티 | 해석 | 하드웨어 동작 |
| --- | --- | --- | --- |
| `0` | 일치 | 오류 없음 | 그대로 통과 |
| `≠0` | 불일치 | 홀수 개 뒤집힘 = 단일 오류. 신드롬이 위치를 정확히 가리킴 | 그 비트만 도로 뒤집어 **정정**. `Correctable` +1 (**SEC**) |
| `≠0` | 일치 | 짝수 개 뒤집힘 = 이중 오류. 신드롬은 엉뚱한 곳을 가리킴 | **정정 포기**, 오염 신고. `Uncorrectable` +1, Xid 48 (**DED**) |
| `0` | 불일치 | 전체 패리티 비트 자신이 뒤집힘 | 데이터는 무사 |

세 번째 줄이 DED의 정체다. "뭔가 났다는 건 아는데 어디인지는 알 수 없다"는 상태이고, 그래서 **DED는 검출까지만 하고 정정은 하지 않는다**. 못 하는 것이지 안 하는 게 아니며, 억지로 하면 데이터를 더 망가뜨린다.

참고로 세 비트가 뒤집히면 개수가 홀수라 **단일 오류처럼 보이고**, 신드롬이 가리키는 엉뚱한 자리를 조용히 "정정"해 버린다. SECDED가 보장하는 건 딱 단일 정정·이중 검출까지이고, 그 이상은 한 워드 안에서 동시에 일어날 확률이 충분히 낮다고 보고 감수하는 설계다.

> 참고: 여기까지가 운영에서 필요한 경계다. 실제 하드웨어는 이 감시 관계를 검사 행렬(parity-check matrix) `H` 하나로 표현해 두고, 읽어 온 워드에 `H`를 곱해 신드롬을 한 번에 뽑는다. 위 도식처럼 패리티를 하나씩 따져 보는 게 아니라 XOR 트리 한 방이라, 매 메모리 접근마다 돌려도 지연이 거의 없다. 왜 최소 거리(Hamming distance)가 4여야 SECDED가 성립하는지, 실제 `(72,64)` 구현이 왜 순수 Hamming이 아니라 Hsiao code인지 같은 이야기는 부호 이론(coding theory) 영역이다. 더 파고 싶으면 Hamming distance, parity-check matrix, Hsiao code 키워드로 시작하면 된다.
