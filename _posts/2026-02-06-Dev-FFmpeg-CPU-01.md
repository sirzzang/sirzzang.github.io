---
title:  "[Kubernetes] Pod CPU Limit과 FFmpeg Thread 최적 조정 - 1. 배경지식: CPU와 스케줄링"
...
컴퓨팅 자원은 초과 사용 시의 동작에 따라 두 종류로 나뉜다.
- **Compressible resource**(압축 가능 자원): 초과 시 프로세스가 종료되지 않고 속도만 저하되는, 시간을 나눠 쓸 수 있는 자원이다. CPU가 대표적이다.
- **Incompressible resource**(비압축 자원): 한 프로세스가 점유하면 다른 프로세스가 사용할 수 없고, 한계를 넘으면 프로세스가 강제 종료되는 공간 자원이다. 메모리가 대표적이다.

CPU와 메모리는 둘 다 컴퓨팅 자원이지만, 이 구분에 따라 성격이 다르다.
...
CPU 코어가 1개인 시스템에서는 한 시점에 하나의 스레드만 실행할 수 있다. 여러 스레드가 있으면 커널이 빠르게 전환하면서 번갈아 실행한다. 이 전환을 **컨텍스트 스위치(context switch)**라고 하며, 현재 실행 중인 스레드의 상태(레지스터, 프로그램 카운터 등)를 저장하고 다음 스레드의 상태를 복원하는 과정이다. 동시에 실행되는 것처럼 보이지만, 실제로는 시간을 나눠쓰는 것이다. 컨텍스트 스위치 자체도 CPU 시간을 소비하므로, 스레드가 과도하게 많으면 전환 비용(캐시 무효화, TLB flush 등)이 누적되어 실질적인 작업 처리량이 줄어들 수 있다.
...
| 예시 | 코어 1개에서 ffmpeg의 여러 스레드가 컨텍스트 스위치로 번갈아 실행 | 멀티코어에서 ffmpeg의 여러 스레드가 각 코어에서 동시에 실행 |
...
### throttling이 발생하는 환경에서의 해석

> 아래에서 다룰 [throttling](#throttling-quota-소진-시-강제-대기)이 발생하면 이 비율의 해석이 달라진다. throttling의 정확한 메커니즘은 해당 섹션에서 다루며, 여기서는 해석상 주의점만 미리 짚어 둔다.

**throttling이 발생하는 환경에서는 이 비율의 해석에 주의가 필요하다**. 예를 들어 quota = 100ms, period = 100ms(1코어 제한) 환경에서 4개의 스레드가 동시에 실행되는 경우를 보자.

```
1 period (100ms) 동안:
  4스레드 동시 실행 → 벽시계 25ms 동안 각 스레드가 25ms씩 CPU 사용
  CPU 시간 합산: 4 × 25ms = 100ms → quota 소진
  나머지 75ms: 전원 대기 (throttling)

10 period (1초) 동안:
  real  = 1,000ms (벽시계)
  user + sys = 100ms × 10 periods = 1,000ms (매 period마다 quota만큼만 일함)
  비율 = 1,000 / 1,000 = 1.0
```

비율이 1.0이므로 "1코어만 사용했다"로 읽기 쉽지만, 실제로는 4코어를 사용하면서 throttling에 의해 실효적으로 1코어 분량의 작업만 수행한 것이다. quota가 ceiling 역할을 하기 때문에, 스레드가 몇 개든 `(user+sys)/real`은 장기적으로 `quota/period`(= limit 코어 수)에 수렴한다. 즉, throttling 환경에서 이 비율은 **실제 사용 코어 수가 아닌 실효 CPU 활용률**에 가깝다.
...
**CFS(Completely Fair Scheduler)**는 각 프로세스의 가상 실행 시간을 추적하여, 모든 프로세스에 CPU 시간을 균등하게 배분하는 리눅스 기본 스케줄러다. 이름 그대로 *완전히 공정한 스케줄러*다. 모든 프로세스에 CPU 시간을 균등하게 배분하는 것을 목표로 한다.
...
CFS가 "공정하게" CPU를 나눠준다고 했다. 그런데 특정 프로세스 그룹이 CPU를 무한정 쓰지 못하도록 상한을 걸고 싶다면? **CFS bandwidth control**이 이 역할을 한다. CFS bandwidth control은 일정 주기(period) 내에 프로세스 그룹이 사용할 수 있는 CPU 시간의 상한(quota)을 설정하는 메커니즘이다.
...
quota를 다 쓰면 어떻게 될까? 해당 그룹의 모든 프로세스(스레드 포함)는 다음 period가 시작되어 quota가 다시 충전될 때까지 **강제로 대기**한다. 이것이 **CPU throttling**이다. 

즉, throttling이란 CFS bandwidth control에서 프로세스 그룹이 할당된 quota를 모두 소진했을 때, 커널이 다음 period까지 해당 그룹의 모든 스레드 실행을 강제로 중단시키는 것이다. 커널이 cgroup의 quota 소비를 추적하다가, quota가 0이 되면 해당 그룹의 모든 태스크를 런큐에서 제외하여 강제로 실행을 중단시키는 것이다. 다음 period가 시작되어 quota가 재충전되면, 태스크들은 다시 런큐에 복귀하여 실행을 재개한다.
...
여기에 앞서 다룬 컨텍스트 스위치 비용도 더해진다. quota가 제한된 상태에서 과도한 스레드가 경합하면, 스레드 간 전환에 소비되는 CPU 시간도 quota에서 차감된다. 즉, 실제 작업에 쓸 수 있는 유효 quota가 줄어드는 이중의 불이익이 발생한다.
...
> 이 메커니즘을 이해하고 나면, 멀티스레드가 항상 유리하다는 통념이 재고된다. CPU quota가 제한된 컨테이너 환경에서는 스레드 수를 늘리는 것이 오히려 성능을 악화시킬 수 있다. 스레드 수와 가용 CPU 자원의 균형이 핵심이다.
