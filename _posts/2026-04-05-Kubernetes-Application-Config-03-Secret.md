...
민감 데이터(비밀번호, API 키, TLS 인증서 등)는 어플리케이션 설정 중 가장 다루기 까다로운 부분이다. 코드에 하드코딩하면 Git 히스토리에 남고, 이미지에 넣으면 레이어에 잔류하며, ConfigMap에 저장하면 etcd에 평문으로 남는다.

Kubernetes는 이 문제를 **Secret**이라는 별도 API 오브젝트로 해결한다. ConfigMap과 별도로 존재하는 설계적 이유는 명확하다.

- **관심사 분리**: 민감 데이터와 일반 설정의 접근 권한을 RBAC으로 독립 제어할 수 있다
- **저장/전송 차별화**: Secret은 노드에서 tmpfs(메모리)에만 저장되고, etcd 암호화를 별도로 적용할 수 있다
- **감사(audit) 추적**: Secret 접근을 별도로 로깅하여 보안 감사에 활용할 수 있다

<br>

# Secret 소개

ConfigMap에 자격 증명이나 암호화 키 같은 민감 데이터도 저장할 수 있다고 생각할 수 있지만, 이는 최선의 방법이 아니다. 보안이 필요한 데이터를 위해 쿠버네티스는 **Secret**이라는 별도의 오브젝트 유형을 제공한다. Secret은 비밀번호, OAuth 토큰, SSH 키 같은 소량의 민감 데이터를 저장하며, 어플리케이션 코드에 기밀 데이터를 포함하지 않아도 되게 해준다. ([Kubernetes 공식 문서: Secrets](https://kubernetes.io/docs/concepts/configuration/secret/))
...
| 내장 Secret 유형 | 설명 |
| --- | --- |
| **`Opaque`** (범용) | 임의의 키에 시크릿 데이터를 저장하는 기본 유형. `type` 필드 없이 Secret을 생성하면 Opaque로 생성됨 |
...
> ConfigMap과 마찬가지로 Secret의 최대 크기는 약 1MiB이다. 이는 etcd의 키-값 쌍 크기 제한(1MiB)에서 비롯되며, etcd에 대한 각 요청은 1.5MiB로 제한된다. ([etcd breaks at scale](https://learnkube.com/etcd-breaks-at-scale))
...
Secret에 민감 데이터를 저장하는 것이 ConfigMap보다 낫지만, Secret이 생각만큼 안전하지 않을 수 있다. Secret의 보안 한계를 **위협 모델** 관점에서 정리하면 다음과 같다.
...
### CI/CD 파이프라인과 컨테이너 이미지 레이어에서의 민감 정보 노출

실무에서 간과하기 쉬운 보안 위협이 두 가지 더 있다.

- **CI/CD 파이프라인 민감 정보 노출**: 빌드/배포 파이프라인에서 환경 변수나 설정 파일에 민감 정보를 소홀히 다루면, 빌드 로그나 아티팩트를 통해 노출될 수 있다. [Kubernetes Goat: Sensitive Keys in Codebases](https://madhuakula.com/kubernetes-goat/docs/scenarios/scenario-1/sensitive-keys-in-codebases-in-kubernetes-containers/welcome)에서 이 시나리오를 실습할 수 있다
- **컨테이너 이미지 레이어에 잔류하는 민감 정보**: 이미지 빌드 과정에서 특정 레이어에 민감 정보가 포함되었다가 이후 레이어에서 해당 파일을 삭제하더라도, 이전 레이어를 복구하면 민감 정보를 탈취할 수 있다. [Kubernetes Goat: Hidden in Container Layers](https://madhuakula.com/kubernetes-goat/docs/scenarios/scenario-15/hidden-in-container-layers-in-kubernetes-cluster/welcome)에서 이 시나리오를 확인할 수 있다

## 외부 Secret 관리 도구로 보완

위 문제들의 권장 해결책은 HashiCorp Vault 같은 전용 외부 Secret 관리 도구를 사용하는 것이다. 강력한 암호화, 세분화된 접근 제어, 자동 Secret 교체, 상세한 감사 로그, 동적 Secret 생성을 제공한다. ([CloudNet@ Hashicorp Vault + VSO on K8S 실습](https://gasidaseo.notion.site/Hashicorp-Vault-VSO-on-K8S-1d850aec5edf809abf22fe0dc7f9d887))

> **실무 관점**: 보안팀에서 Secret을 애초에 암호화하여 입력하라고 권고하는 경우가 있지만, DevOps/플랫폼팀 입장에서는 Vault 같은 외부 Secret 관리 도구를 도입하는 것이 가장 현실적이다. 암호화 입력 방식은 키 관리와 배포 파이프라인의 복잡도를 높이는 반면, Vault는 동적 Secret 생성, 자동 교체, 감사 로그를 통합적으로 제공하기 때문이다.
...
- **Secret 볼륨**: 워커 노드의 **tmpfs(RAM 기반 파일시스템)**에만 저장된다. 디스크에 절대 기록되지 않으며, 노드가 꺼지면 데이터가 사라진다. 다만, 노드에 swap이 활성화되어 있으면 tmpfs 데이터가 swap 파티션(디스크)에 쓰일 수 있다는 점을 유의해야 한다
...
- **ConfigMap**: 해당 ConfigMap을 참조하는 파드가 실행 중인 노드에 배포된다. kubelet의 ConfigMap 캐시 정책에 따라 동작이 달라질 수 있으며, 파드가 없는 노드에서도 캐시가 남아있을 수 있다는 의견과 파드가 기동된 노드에만 존재한다는 의견이 있다 <!-- TODO: ConfigMap 노드 배포 동작에 대한 추가 검증 필요 -->
- **Secret**: 해당 Secret을 사용하는 파드가 실행 중인 노드에만 배포된다. 파드가 종료되면 kubelet이 Secret의 로컬 복사본을 삭제한다

<br>

# 실무 관점: Secret 관리 도구 생태계

Kubernetes 내장 Secret만으로는 위에서 살펴본 보안 한계를 모두 해결할 수 없다. 실무에서는 다음 도구들로 보완한다.

| 도구 | 해결하는 위협 | 동작 방식 | 특징 |
| --- | --- | --- | --- |
| **Bitnami Sealed Secrets** | 저장 시 — 매니페스트 평문 노출 | SealedSecret CR을 암호화하여 Git에 커밋. 클러스터 내 컨트롤러가 복호화하여 Secret 생성 | GitOps 친화적 |
| **External Secrets Operator** | 접근 시 — Secret 소스 외부화 | AWS Secrets Manager, GCP Secret Manager 등 외부 저장소와 동기화 | 멀티 클라우드 지원 |
| **Secrets Store CSI Driver** | 전송 시 — etcd 미경유 | 외부 저장소의 Secret을 CSI 볼륨으로 직접 마운트 | etcd에 Secret이 저장되지 않음 |
| **HashiCorp Vault** | 전체 — 동적 Secret, 자동 교체, 감사 로그 | Vault Agent/Injector가 파드에 Secret 주입 | 가장 포괄적이지만 운영 복잡도 높음 |

어떤 도구를 선택할지는 팀의 보안 요구 수준, 기존 인프라(클라우드 제공자의 Secret Manager 유무), GitOps 워크플로우 여부에 따라 달라진다.
