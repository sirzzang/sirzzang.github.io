---
title:  "[Kubernetes] GPU Sharing: Time Slicing - 2. 설정"
...
- 쿠버네티스에서 GPU Time Slicing은 **NVIDIA Device Plugin이 GPU 개수를 실제보다 많게 보고(overcommit)**하는 방식으로 동작한다
- 동작 흐름은 2단계로 나뉜다: (1) Device Plugin이 ConfigMap의 replicas 수만큼 논리적 GPU를 Kubelet에 보고 (ListAndWatch), (2) 파드 스케줄링 시 동일한 물리 GPU 정보를 반환 (Allocate)
- **커널 드라이버(nvidia.ko)는 ConfigMap의 존재를 모른다**. 단지 여러 GPU Context가 감지되면 자동으로 Time Slicing을 수행한다
- ConfigMap의 핵심 설정: `replicas`(GPU당 논리적 분할 수), `renameByDefault`(리소스명 변경), `failRequestsGreaterThanOne`(다중 GPU 요청 차단)

<br>

[지난 글]({% post_url 2025-11-22-Kubernetes-GPU-Time-Slicing-1 %})에서 GPU Time Slicing의 일반적인 개념에 대해 알아보았다. Context Switching, Preemptive Switching, 그리고 ~~간략한~~ 하드웨어 수준에서의 동작 원리까지 살펴보았는데, 이번 글에서는 이러한 개념이 쿠버네티스 환경에서 **어떻게 동작하는지**, 그리고 **Time Slicing ConfigMap을 어떻게 구성하는지** 알아본다.

GPU Time Slicing을 적용하는 방법은 GPU 벤더마다 다르다. 이 글에서는 가장 널리 사용되는 **NVIDIA GPU**를 기준으로 설명한다. NVIDIA는 쿠버네티스 환경에서 GPU를 사용할 수 있게 해주는 [NVIDIA Device Plugin]({% post_url 2024-07-19-Dev-Kubernetes-GPU-Setting %})과 [GPU Operator](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/overview.html)를 제공하며, 이를 통해 Time Slicing 설정을 적용할 수 있다.
...
쿠버네티스 환경에서 GPU Time Slicing이 어떻게 동작하는지 알아보자. [NVIDIA Device Plugin 동작 원리]({% post_url 2024-07-23-Dev-Kubernetes-NVIDIA-GPU-Mechanism %})에서 Device Plugin이 Kubelet에게 GPU 개수를 보고하고, 스케줄링 시 GPU를 할당하는 메커니즘을 소개한 바 있다. Time Slicing은 이 메커니즘을 활용하되, 핵심은 **NVIDIA Device Plugin이 GPU 개수를 속여서 보고**한다는 것이다.
...
[1] ConfigMap을 읽어 replicas 수만큼 논리적 GPU 생성
[2] Kubelet에 GPU 개수 보고 (ListAndWatch)
```
...
┌──────────────▼────────────────────────┐
│   NVIDIA Device Plugin                │
│   - return same physical GPU info     │
└──────────────┬────────────────────────┘
               │ [3]
┌──────────────▼────────────────────────┐
│   Container Runtime                   │
│   - mount same GPU to containers      │
└──────────────┬────────────────────────┘
               │ [4] CUDA init
┌──────────────▼────────────────────────┐
│   nvidia.ko (kernel driver)           │
│   - unaware of ConfigMap              │
│   - detect multiple contexts          │
│   - auto time-slicing                 │
└───────────────────────────────────────┘

[1] Kubelet이 Device Plugin에 GPU 할당 요청
[2] Device Plugin이 동일한 물리 GPU 정보를 반환 (Allocate)
[3] Container Runtime이 같은 GPU를 여러 컨테이너에 마운트
[4] 커널 드라이버는 ConfigMap의 존재를 모른다. 단지 여러 GPU Context가 감지되면 자동으로 Time Slicing을 수행
```
...
[NVIDIA Device Plugin이 GPU 개수를 보고]({% post_url 2024-07-23-Dev-Kubernetes-NVIDIA-GPU-Mechanism %}#보고)할 때, Time Slicing 설정의 replica 수에 따라 **논리 슬롯 개수를 여러 개로 보고**한다.
...
[NVIDIA Device Plugin이 GPU 할당 결과를 반환]({% post_url 2024-07-23-Dev-Kubernetes-NVIDIA-GPU-Mechanism %}#스케줄링)할 때, **전부 같은 물리 GPU를 반환**한다.
...
* 하나의 물리 GPU를 시간 단위로 분할하여 Pod A, B, C가 번갈아 사용한다.
```

각 파드는 자신이 전용 GPU를 받았다고 생각하지만, 실제로는 동일한 GPU를 보고 있으며, NVIDIA GPU Driver의 [Compute Preemption]({% post_url 2025-11-22-Kubernetes-GPU-Time-Slicing-1 %}#preemptive-context-switching)이 시간을 나눠 할당해 준다.
...
이 글에서는 쿠버네티스 환경에서 GPU Time Slicing이 어떻게 동작하고, ConfigMap을 어떻게 구성하는지 살펴보았다.

| 항목 | 내용 |
|------|------|
| **동작 핵심** | Device Plugin이 replicas 수만큼 논리적 GPU를 보고하여 overcommit |
| **1단계 (ListAndWatch)** | ConfigMap → Device Plugin → Kubelet에 GPU 개수 보고 |
| **2단계 (Allocate)** | 파드 스케줄링 시 동일한 물리 GPU를 여러 컨테이너에 할당 |
| **Time Slicing 실행 주체** | 커널 드라이버(nvidia.ko)가 여러 Context 감지 시 자동 수행 |
| **주요 ConfigMap 필드** | `replicas`, `renameByDefault`, `failRequestsGreaterThanOne` |

[다음 글]({% post_url 2025-11-22-Kubernetes-GPU-Time-Slicing-3 %})에서는 실제로 Time Slicing을 적용하는 방법과 적용 사례, 그리고 운영 시 주의해야 할 한계점들을 살펴본다.
