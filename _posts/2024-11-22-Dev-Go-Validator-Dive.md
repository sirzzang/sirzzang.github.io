...
([gin을 이용해 친절한(?) 에러 메시지 보내기](https://sirzzang.github.io/dev/Dev-Go-Validator-Custom-Error-Message/){: .btn .btn--success .btn--small})에서 알아 봤듯, gin 프레임워크를 이용하면 Request Body를 원하는 구조체로 쉽게 binding할 수 있다. 검증까지 포함해서!
...
먼저 gin의 모델 바인딩은 어떻게 동작하는지 살펴 보자. 위의 예시에서 작성한 `ShouldBindJSON`의 소스 코드를 보면, `binding.Binding` 인터페이스를 구현한 `binding.Binding` 타입 인스턴스의 `Bind` 메서드를 구현해 모델 바인딩을 진행하는 것을 확인할 수 있다.
...
> *참고*: key, endkey
>
> `key`, `endkey`를 이용하면 map의 key에 대해서만 validation을 적용할 수 있다고 한다. 
...
아래와 같이 `binding` 태그에 `dive`만 추가해 주면 원하는 대로 동작하게 된다.
