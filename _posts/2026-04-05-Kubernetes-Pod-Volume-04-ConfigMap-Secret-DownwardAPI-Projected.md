...
[어플리케이션 설정]({% post_url 2026-04-05-Kubernetes-Application-Config-01-Command-Args-Env %}) 시리즈에서 ConfigMap, Secret, Downward API의 데이터를 **환경 변수**로 컨테이너에 주입하는 방법을 다뤘다. 이 글에서는 같은 데이터를 **볼륨을 통해 파일로** 컨테이너에 주입하는 방식을 정리한다.

## 환경 변수 vs 볼륨: 언제 어떤 방식을 쓸 것인가

| 판단 기준 | 환경 변수 | 볼륨 마운트 |
| --- | --- | --- |
| 데이터 형태 | 짧은 단일 행 값 (포트, URL 등) | 여러 줄 설정 파일 (`nginx.conf`, `envoy.yaml` 등) |
| 런타임 자동 반영 | X (컨테이너 재시작 필요) | O (심볼릭 링크 교체, subPath 제외) |
| 보안 | 로그/자식 프로세스에 노출 위험 | tmpfs + 파일 권한으로 보호 가능 |
| 접근 방식 | `process.env` 등 표준 API | 파일 읽기 |

환경 변수 방식은 작은 단일 행 값에 적합하고, 긴 여러 줄의 설정 파일은 볼륨으로 전달하는 것이 더 적합하다.
...
`configMap` 볼륨은 ConfigMap 항목을 개별 파일로 사용할 수 있게 해준다. ConfigMap의 `data` 필드에 있는 각 키가 볼륨 내 파일 이름이 되고, 값이 해당 파일의 내용이 된다. ([Kubernetes 공식 문서: Using ConfigMaps as files from a Pod](https://kubernetes.io/docs/concepts/configuration/configmap/#using-configmaps-as-files-from-a-pod)) 컨테이너에서 실행 중인 프로세스가 이 파일들을 읽어 값을 가져올 수 있다. 주로 대용량 설정 파일을 컨테이너에 전달하는 데 사용되지만, 작은 값에도 사용할 수 있으며, `env`나 `envFrom` 필드와 결합하여 큰 항목은 파일로, 나머지는 환경 변수로 전달할 수도 있다.
...
configMap 볼륨의 동작 방식을 이해해 두지 않으면 예상치 못한 동작을 디버깅하는 데 많은 시간을 소비하게 될 수 있다. 볼륨 마운트의 기본 동작(디렉토리 마운트 vs subPath, 심볼릭 링크 기반 원자적 업데이트)은 [볼륨 소개]({% post_url 2026-04-05-Kubernetes-Pod-Volume-01-Introduction %}) 편에서 다뤘다. 여기서는 configMap 볼륨 특유의 동작에 집중한다.
...
- Pod 내 **모든 컨테이너의 프로세스**에 (기존 그룹에 더해) **supplemental group(보조 그룹)**으로 추가한다
...
- `fsGroup`은 Linux의 supplemental group(보조 그룹) 메커니즘을 활용한다. 서로 다른 UID로 실행되는 여러 컨테이너가 **같은 그룹을 공유**하면서 볼륨 파일에 접근할 수 있다
...
지금까지 ConfigMap, Secret, 그리고 Pod 오브젝트 자체의 값을 주입하기 위해 세 가지 볼륨 타입을 사용하는 방법을 봤다. ([Kubernetes 공식 문서: Projected Volumes](https://kubernetes.io/docs/concepts/storage/projected-volumes/)) `volumeMount` 정의에서 `subPath` 필드를 사용하지 않는 한, 서로 다른 소스의 파일들을 **동일한 파일 디렉토리에 주입**할 수 없다. 이때 `projected` 볼륨을 사용한다.
...
# 최종 의사결정: 데이터 소스 x 전달 방식 매트릭스

[어플리케이션 설정]({% post_url 2026-04-05-Kubernetes-Application-Config-01-Command-Args-Env %}) 시리즈와 이 시리즈를 관통하는 핵심 의사결정을 하나의 표로 정리한다.

| 데이터 소스 | 환경 변수 (`env`/`envFrom`) | 볼륨 마운트 | 자동 반영 (볼륨) | 저장 매체 (볼륨) |
| --- | --- | --- | --- | --- |
| **리터럴 값** | `env[].value` | — | — | — |
| **ConfigMap** | `configMapKeyRef` / `envFrom` | `configMap` 볼륨 | O (subPath 제외) | 디스크 |
| **Secret** | `secretKeyRef` / `envFrom` | `secret` 볼륨 | O (subPath 제외) | tmpfs (메모리) |
| **Downward API** | `fieldRef` / `resourceFieldRef` | `downwardAPI` 볼륨 | O (labels/annotations) | 디스크 |
| **복합 소스** | — | `projected` 볼륨 | O | 소스별 상이 |

**선택 원칙:**
- 짧은 값, 프로세스 환경에서 바로 읽어야 하는 경우 → 환경 변수
- 설정 파일 형태, 런타임 자동 반영이 필요한 경우 → 볼륨 마운트
- Secret은 가급적 볼륨 마운트 (환경 변수는 로그/자식 프로세스 노출 위험)
