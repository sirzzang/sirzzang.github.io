...
- Neuron SDK는 CUDA 자리에 놓이는 소프트웨어 계층이지만 구조가 같지는 않다. 서빙 경로가 AOT 컴파일을 전제로 하고, 산출물(NEFF)이 커널 바이너리가 아니라 그래프 전체의 실행 계획이며, 커널 라이브러리 선택이 런타임 dispatch가 아니라 컴파일 타임 결정이다
...
세는 단위만 다른 것이 아니라 판단 기준도 다르다. 학습에서는 처리량 절댓값만으로 좋고 나쁨을 알 수 없어, 하드웨어 이론 최대 성능 대비 얼마를 실제로 썼는지로 환산한 MFU(Model FLOPs Utilization)를 함께 본다. 근거가 되는 근사식이 6ND rule이다. 파라미터 수 `N`, 학습 토큰 수 `D`일 때 학습 연산량을 약 `6ND` FLOPs로 잡는 어림인데, 토큰 하나 기준으로 보면 forward가 약 `2N`, backward가 그 2배인 `4N`이라 합계가 `6N`이고 추론은 forward만이므로 `2N`이다. 앞 표의 "학습 연산량이 추론의 3배"가 여기서 나온다. 다만 이건 가중치 행렬곱만 센 어림이라 어텐션·softmax·LayerNorm을 빼고 있고, 컨텍스트가 길어질수록 오차가 커진다. activation recomputation을 쓰면 forward를 한 번 더 돌리므로 `6N`도 성립하지 않는다.
...
Inferentia1(Inf1 인스턴스)은 이름 그대로 추론 전용 칩이다.
...
1. **메모리 용량 3배, 대역폭 3.5배** — decode는 산술 강도가 낮아 HBM 대역폭이 상한이다([roofline 정리]({% post_url 2026-08-21-Dev-Roofline-Model-LLM-Serving %})). 이 축이 LLM 추론 성능에 가장 직접 닿는다
2. **칩당 코어 2개에서 8개로** — 칩 경계를 넘지 않고 칩 하나 안에서 텐서 병렬을 8까지 걸 수 있다
3. **FP8 6.8배와 희소 연산 신규** — 양자화 추론 경로에 집중 투자한 흔적이다
...
NVIDIA는 아키텍처 레벨에서 학습용과 추론용이 나뉘지 않는다. 같은 CUDA 프로그래밍 모델 위에서 다이 크기·메모리·인터커넥트·정밀도 유닛을 조절해 여러 제품 SKU로 분화시킨다. 다만 계보가 하나인 것은 아니다. 데이터센터 대형 다이가 Ampere → Hopper → Blackwell로 이어지는 동안, 아래 표의 추론 지향 SKU는 T4가 Turing, L4·L40S가 Ada로 그래픽 계열 아키텍처에서 갈라져 나온다.

| | 학습 지향 (H100/H200/B200) | 추론 지향 (T4/L4/L40S) |
| --- | --- | --- |
| 메모리 종류 | HBM (80~192 GB, 3~8 TB/s) | GDDR6 (16~48 GB, 0.3~0.86 TB/s) |
...
3. **MoE가 Expert Parallelism을 요구한다.** 전문가를 여러 칩에 나눠 배치해야 하므로 다중 칩 토폴로지가 추론에서도 기본이 된다
...
AWS 문서는 Trn1 인스턴스의 구성을 이렇게 적는다.
...
flowchart TD
    ...
    CHIP --> CORE0["NeuronCore-v2 0번<br/>런타임 코어 인덱스 0"]
    CHIP --> CORE1["NeuronCore-v2 1번<br/>런타임 코어 인덱스 1"]
...
| 디바이스 노드 | `/dev` | `neuron0` 1개. 코어는 별도 노드로 나뉘지 않는다 |
...
엔진 구성을 학습·추론 관점으로 읽으면 학습 지향의 흔적이 남아 있다. GPSIMD-Engine은 정형 연산에서 벗어난 커스텀 오퍼레이터를 얹을 수 있게 하는 유연성 장치다. [NKI](#nki)로 작성한 커널은 이 엔진만이 아니라 Tensor·Vector·Scalar·GpSimd 네 엔진 전부를 타깃으로 할 수 있다. 여기에 Trainium 고유 기능으로 하드웨어 stochastic rounding(BF16 누적 시 편향 제거)과 연산·통신 오버랩용 collective 엔진이 들어 있다. GPSIMD와 stochastic rounding은 추론만 할 것이라면 굳이 없어도 되는 기능들이다.
...
NeuronLink는 Trainium과 Inferentia2가 같은 아키텍처를 쓴다. PCIe를 경유하지 않고 칩끼리 전용 링크로 직접 통신한다는 점에서 NVLink와 같은 자리에 놓인다. 대역폭을 비교할 때는 방향 기준을 맞춰야 한다. NVLink 4세대의 GPU당 900 GB/s는 양방향 합산값이고, AWS가 Inf2에 대해 적은 값은 칩당 192 GiB/s다. Trainium1은 링크가 넷이라 여기서 384 GB/s가 나오는데, EC2 Trn1 제품 페이지는 같은 칩의 NeuronLink 대역폭을 최대 768 GB/s로 적는다. 어느 값을 쓰느냐에 따라 격차가 크게 달라지므로 한쪽 숫자만 들어 급을 단정하기는 어렵다. 토폴로지도 다르다. `trn1.32xlarge`의 16칩은 스위치가 아니라 2D Torus로 엮여 있어, 홉 수가 노드 내 위치에 따라 달라진다.

학습 throughput을 떨어뜨리는 요인 중 하나가 collective 통신이 연산과 겹치지 않아 생기는 idle 시간인데, Trainium은 NeuronLink-v2와 전용 collective 엔진으로 이 오버랩을 하드웨어에서 지원한다.
...
`neuronx-cc`는 `nvcc`나 XLA/TorchInductor가 놓이는 자리에 있지만 동작이 다르다. 서빙 경로가 AOT(ahead-of-time) 컴파일을 전제로 한다. 모델 그래프를 미리 컴파일해 두어야 실행할 수 있고, 그래서 정적 shape이 전제가 된다. 실습에서 `MAX_MODEL_LEN`이나 버킷 설정을 미리 고정하는 이유가 여기에 있다.

다만 Neuron에 JIT 경로가 아예 없는 것은 아니다. AWS 문서는 XLA Lazy Tensor 방식이 Neuron 실행에 JIT 컴파일을 쓴다고 적는다. 없는 것은 cuBLAS·cuDNN처럼 **커널 단위로 런타임에 구현을 고르는 dispatch**이지 JIT 컴파일 자체가 아니다.
...
런타임은 `libnrt.so`이고 CUDA Driver API(`libcuda.so`)와 같은 자리다. 디바이스 열기, NEFF 적재, 실행 요청 큐잉, collective 통신을 담당한다. EFA 연동에 필요한 OFI 플러그인이 `aws-neuronx-collectives` 패키지 안에 미리 빌드되어 들어 있어, NCCL과 EFA를 잇는 플러그인을 따로 받아 빌드하는 단계가 없다.
...
굳이 커널 레벨까지 내려가야 하는 이유는 하드웨어 쪽에 이미 슬롯이 있기 때문이다. [GPSIMD-Engine](#neuroncore-v2의-네-엔진)이 정형 연산 밖의 커스텀 오퍼레이터를 받아 주고, NKI는 소프트웨어 편의 기능이 아니라 네 엔진을 직접 겨냥할 수 있게 열어 주는 창구다. 실제로 필요해지는 시나리오는 네 가지 정도다.
...
- **EKS Neuron optimized AMI** — EKS 워커 노드용이고 Neuron 드라이버를 포함한다. `/aws/service/eks/optimized-ami/1.33/amazon-linux-2023/x86_64/neuron/recommended/image_id` SSM 파라미터로 조회한다. 이 AMI를 썼기 때문에 워커 노드에서 `lsmod` 결과에 neuron 모듈이 이미 올라와 있고 `/dev/neuron0`이 존재한다
...
DLAMI도 변형이 여럿이라는 점은 같다. 프레임워크·OS·Base 여부로 갈리므로 **"아무 DLAMI나 고르면 다 된다"는 아니다**. 다른 것은 두 가지다.
- 첫째, DLAMI는 커널 드라이버까지 책임지는 반면 DLC는 [0]층을 담을 수 없다. 
- 둘째, DLAMI의 프레임워크 계층은 venv 안 pip 패키지라 빠진 것을 나중에 설치해 메울 수 있는 반면, DLC는 이미지 태그가 곧 스택 구성이라 잘못 고르면 되돌릴 방법이 이미지 교체뿐이다.
...
| NeuronLink-v2 | NVLink | 역할은 정확히 대응. 대역폭은 문서마다 기준이 달라(칩당 384 GB/s ~ 768 GB/s 대 GPU당 900 GB/s 양방향) 직접 비교에 주의가 필요하다 |
...
| `aws-neuronx-collectives`에 OFI 플러그인 내장 | `aws-ofi-nccl` | 별도 설치·빌드 단계가 없다 |
...
| `neuron-bench` | 마이크로벤치마크 | NEFF를 반복 실행해 종단 지연·처리량 통계 측정 |
...
`nvcc`는 AOT 컴파일과 런타임 JIT 경로를 함께 갖고, cuBLAS·cuDNN은 런타임에 입력 shape을 보고 커널을 고른다. Neuron에서는 이 결정이 컴파일 타임으로 옮겨간다. NEFF는 그래프 전체의 실행 계획이고, 어떤 커널을 쓸지도 컴파일 시점에 정해진다. 정적 shape을 미리 확정해야 하는 제약이 여기서 나오고, 대신 NEFF 캐시로 재기동 비용을 줄일 수 있다.
...
NVIDIA 스택에 있던 층 둘이 Neuron에는 없다. NCCL과 EFA를 잇는 `aws-ofi-nccl`에 대응하는 OFI 플러그인은 `aws-neuronx-collectives`에 미리 빌드되어 들어 있어 따로 설치할 것이 없다. 컨테이너에 GPU를 노출하는 `nvidia-container-toolkit`도 필요 없고, 표준 runc와 device plugin만으로 처리된다([device plugin 동작 흐름]({% post_url 2026-06-07-Kubernetes-GenAI-on-K8s-10-01-GPU-Resources-and-K8s-Allocation %}#device-plugin-동작-흐름), [커스텀 가속기]({% post_url 2026-06-07-Kubernetes-GenAI-on-K8s-10-01-GPU-Resources-and-K8s-Allocation %}#커스텀-가속기)).
...
| | vLLM + NxD Inference | vLLM Neuron (베타) |
| --- | --- | --- |
| 모델 실행 | `neuronx-distributed-inference` 라이브러리 | 플러그인 안에 모델 구현 내장 |
| 지원 인스턴스 | Trn1, Trn2, Trn3, Inf2 | Trn2, Trn3 |
| 플러그인 계열 | 0.5.x | 0.24.0 계열 |
| 지원 vLLM | 0.16 | 0.24.0 |
| 특징 | 모델이 NxDI에 온보딩돼 있어야 한다. 업스트림 표기는 `Maintenance`다 | segmented prefill, disaggregated inference, EAGLE3 speculative decoding. 업스트림이 권장 서빙 경로로 적는 쪽이다. chunked prefill은 아직 미지원 |
...
이 환경변수 하나가 "vLLM 플러그인이 NxD Inference를 모델 실행 백엔드로 쓴다"는 선언이다. 이것을 포함한 vLLM ConfigMap 키들의 값과 의미는 [8.3.1편]({% post_url 2026-09-11-Kubernetes-LLM-Serving-Optimization-08-03-01-vLLM-Deployment %})에서 다룬다.
...
여기까지가 칩과 스택 쪽 배경이다. 이 하드웨어를 실제로 쓰는 클러스터를 만드는 작업은 [8.2.1편]({% post_url 2026-09-11-Kubernetes-LLM-Serving-Optimization-08-02-01-EKS-Cluster-Nodegroup %})에서, 칩과 코어가 쿠버네티스 리소스로 광고되는 경로는 [8.2.2편]({% post_url 2026-09-11-Kubernetes-LLM-Serving-Optimization-08-02-02-Neuron-Device-Exposure %})에서 이어 간다.
...
- [NKI (Neuron Kernel Interface)](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/nki/index.html)
- [NKI: Trainium·Inferentia2 아키텍처 가이드](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/nki/get-started/about/index.html)
- [Inference on Neuron (torch-neuronx)](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/frameworks/torch/torch-neuronx/programming-guide/inference/index.html)
