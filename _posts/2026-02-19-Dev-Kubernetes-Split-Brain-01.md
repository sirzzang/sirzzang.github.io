...
네트워크 파티션 시 Split Brain이 발생하는지 여부는 분산 시스템의 설계(quorum 유무, 멤버 수 홀짝 등)에 따라 달라진다. 다만 이번 글의 핵심은 아니므로, 시스템 설계별 비교는 별도의 글에서 다루기로 한다. 여기서 기억할 점은, **어떤 설계든 기존 클러스터와 무관한 별개의 합의 그룹이 생성되면 각각이 독립적으로 쓰기를 수행하게 되어 결과적으로 Split Brain이 된다**는 것이다 — 이번 케이스가 바로 이 경우에 해당한다.
...
> **참고**: K3s에서 `kubectl delete node`는 Kubernetes Node 오브젝트만 삭제하는 것이 아니라, etcd 멤버도 함께 제거한다. K3s의 [`member_controller.go`](https://github.com/k3s-io/k3s/blob/v1.27.9%2Bk3s1/pkg/etcd/member_controller.go)에 구현된 `onRemove` 핸들러가 Node 삭제 이벤트에 반응하여 `etcd RemovePeer`를 호출하기 때문이다. 따라서 "올바른 제거/재조인 절차"에서 2단계로 별도 기술한 `etcdctl member remove`는 실제로 `kubectl delete node`가 자동으로 수행해 준다. 이 메커니즘은 [Part 2]({% post_url 2026-02-19-Dev-Kubernetes-Split-Brain-02 %})에서 소스 코드 분석(실험 5b)으로 검증한다.
