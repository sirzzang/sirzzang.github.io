---
title: "[EKS] EKS: 인증/인가 - 3. 사용자 → K8s API: 전체 흐름"
excerpt: "사용자(IAM)가 kubectl로 EKS API 서버에 접근할 때 인증되고 인가되기까지의 전 과정을 단계별로 따라가 보자."
categories:
  - Kubernetes
toc: true
hidden: true
...
<center><sup>EKS 인증/인가 전체 흐름 (출처: <a href="https://www.youtube.com/watch?v=wgH9xL_48vM">EKS 환경을 더 효율적으로, 더 안전하게 - AWS Summit Korea 2022</a>)</sup></center>
...
TokenReview는 `authentication.k8s.io/v1` API 그룹에 속하는 K8s 리소스로, "이 토큰이 유효한가? 유효하다면 누구의 것인가?"를 묻는 **인증 요청 객체**다.

- **요청(`spec`)**: `spec.token`에 검증할 토큰을 담는다
- **응답(`status`)**: `status.authenticated`(true/false), `status.user`(username, uid, groups, extra)를 돌려준다

TokenReview는 **질의 전용 리소스**로, `create`(POST)만 가능하다. 요청을 보내면 즉시 인증 결과가 응답으로 돌아올 뿐, etcd에 저장되지 않는다. 인가 단계에서 동일한 패턴으로 동작하는 SubjectAccessReview는 [8] 인가에서 다룬다.
...
IAM ARN이 확인되었지만, K8s RBAC은 IAM ARN을 모른다. RBAC이 이해하는 것은 K8s username과 group이다. 따라서 "이 IAM ARN은 K8s에서 어떤 username/group에 해당하는가?"를 매핑해야 한다. 이 단계는 인증의 결과(검증된 IAM identity)를 인가의 입력(K8s username/group)으로 변환하는 **브릿지** 역할을 한다.
...
Access Entry 방안에서는 EKS가 자체 **Webhook 인가**를 추가로 제공한다. 인증에서 API 서버가 TokenReview로 "이 토큰이 유효한가?"를 물었듯이, 인가에서는 **SubjectAccessReview**로 "이 주체가 이 동작을 할 수 있는가?"를 EKS 인가 webhook에 묻는다. Access Policy(EKS 관리형 정책)가 연결되어 있으면 webhook이 이 정책을 기반으로 허용/거부를 판단한다.
...
| **[7] K8S User 확인** | Access Entry 또는 aws-auth | IAM ARN → K8s username/group 매핑 | AuthN → AuthZ 브릿지 |
| **[8] K8S Role 확인** | Webhook + RBAC 또는 RBAC만 | 매핑된 주체의 요청 동작에 대한 권한 확인 | AuthZ |
| **[9] 허용/차단** | K8s API 서버 | 인가 통과 시 요청 실행, 실패 시 403 Forbidden | - |

1~6단계(인증)는 방식이 하나다. 어떤 IAM 주체든 동일한 STS 기반 메커니즘을 거친다. 7단계는 인증 결과를 인가 입력으로 변환하는 브릿지로, 매핑 방안(Access Entry vs aws-auth ConfigMap)에 따라 8단계의 인가 엔진이 달라진다.
...
| **매핑** (AuthN → AuthZ 브릿지) | IAM ARN → K8s username/group. Access Entry(권장) 또는 aws-auth ConfigMap(deprecated). 인증 결과를 인가 입력으로 변환하는 단계 |
| **인가** | Access Entry는 Webhook + RBAC, aws-auth는 RBAC만. 결국 K8s RBAC이 최종 인가를 담당 |

다음 글에서는 인증 흐름([1]~[6])을 실습으로 하나씩 따라가며, 토큰 생성부터 CloudTrail/CloudWatch 검증까지 직접 확인해 본다. 매핑([7])과 인가([8]~[9]) 실습은 별도 글에서 다룰 예정이다.
