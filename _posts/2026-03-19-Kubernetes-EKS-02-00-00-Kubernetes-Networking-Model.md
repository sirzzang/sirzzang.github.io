...
세 방식 모두 파드 간 통신에서 src/dst IP가 한 번도 변하지 않는다. 방법만 다를 뿐 "NAT 없이 파드 IP로 직접 통신"이라는 결과는 동일하다. [다음 글]({% post_url 2026-03-19-Kubernetes-EKS-02-00-01-Pod-to-Pod-Networking %})에서 각 방식을 상세히 다룬다.
...
- [파드 간 통신]({% post_url 2026-03-19-Kubernetes-EKS-02-00-01-Pod-to-Pod-Networking %}): 문제 2의 세 가지 해결 방식 (오버레이, BGP, 클라우드 네이티브)
