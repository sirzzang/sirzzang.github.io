...
- 파이프라인 태스크 간에 전달되기만 하면 되는 데이터는 Argo Workflow의 Artifact를 이용하나, 모델 학습 시 로컬에 존재해야 하는 데이터는 Persistent Volume을 이용한다.
...
- 아래의 값을 Argo Workflow Controller Configmap의 Data 영역에 추가한다.
  ...
  - `config` 키의 값으로 `artifactRepository` 설정 값을 추가한다.
  - `config` 값 내 `artifactRepository.s3.endpoint`으로 MinIO 파드가 배포된 클러스터 내에서 사용할 수 있는 FQDN(Fully Qualified Domain Name) 형태의 DNS를 사용했다.
