...
```
NVRM: Xid (PCI:0000:XX:00): 13, Graphics SM Warp Exception on (GPC 0, TPC 0, SM 0): Out Of Range Address
NVRM: Xid (PCI:0000:XX:00): 13, Graphics SM Global Exception on (GPC 0, TPC 0, SM 0): Multiple Warp Errors
NVRM: Xid (PCI:0000:XX:00): 43, pid=XXXXXX, name=ray::_RayTrainW, channel 0x00000002
```
...
동일 사건의 두 얼굴이다. 근본 원인 후보 중 하나로 지목했던 "CUDA driver와 NCCL 빌드 버전 호환성 불일치"([이전 글]({% post_url 2026-03-29-Articles-NCCL-Troubleshooting-Collaboration-Retrospective %}) 참고)가, sm_120 커널 부재로 구체화된 것이다.

<br>

# 분석: Lazy Init이 디버깅을 어렵게 만든 이유
...
PyTorch 자체는 eager init으로 방향을 전환하고 있지만, Ray TorchTrainer가 이를 따라가기 전까지는 플랫폼 레벨에서 문제를 감지해야 한다.
...
그러나 이 방법은 두 가지 이유로 현실적이지 않다.

**첫째, Ray TorchTrainer에서는 기술적으로 불가능하다.** Ray가 워커 진입 전에 `init_process_group`을 이미 호출하기 때문이다. Ray의 `_TorchBackend.on_start()`는 사용자의 `train_func` 실행 전에 [`_setup_torch_process_group()`](https://github.com/ray-project/ray/blob/master/python/ray/train/torch/config.py)을 호출하고, 이 함수 내부에서 `device_id` 없이 `init_process_group`을 실행한다.

```python
# ray/train/torch/config.py — _setup_torch_process_group()
...
dist.init_process_group(
    ...
    # device_id 미지정 → PyTorch 기본값(lazy init) 적용
)
```

`train_func`이 실행되는 시점에 이미 default process group이 초기화되어 있다. 사용자가 `train_func` 내부에서 다시 `init_process_group`을 호출하면 PyTorch는 에러를 던진다.

```python
# torch/distributed/distributed_c10d.py — init_process_group()
# https://github.com/pytorch/pytorch/blob/main/torch/distributed/distributed_c10d.py#L1660
if GroupMember.WORLD is not None:
    raise ValueError("trying to initialize the default process group twice!")
```

`dist.destroy_process_group()` 후 재초기화하는 우회가 이론적으로는 가능하지만, worker 간 동기화 문제와 Ray 라이프사이클 충돌로 인해 권장하지 않는다.

<details markdown="1">
<summary>destroy/reinit 우회가 위험한 이유</summary>

첫째, 모든 worker의 destroy/reinit 타이밍을 동기화하기 어렵다 — 일부 worker가 destroy를 완료하기 전에 다른 worker가 reinit을 시작하면 NCCL이 invalid state에 빠질 수 있다 ([pytorch/pytorch#119196](https://github.com/pytorch/pytorch/issues/119196), 2026-02 기준 여전히 open).

둘째, Ray Train의 [`on_shutdown`](https://github.com/ray-project/ray/blob/master/python/ray/train/torch/config.py) hook이 별도로 `destroy_process_group()`을 호출하므로, 사용자가 먼저 destroy하면 Ray 측 라이프사이클과 충돌할 수 있다.
...
> 대안 1과 2를 실제로 구현한 과정은 [GPU 호환성 게이트: 빌드 타임과 배포 타임에서 NCCL 커널 미스매치 차단하기]({% post_url 2026-04-30-Dev-NCCL-GPU-Compat-CI-Runtime-Gate %})에서 다룬다.
