...
`dist.destroy_process_group()` 후 재초기화하는 우회 방법이 기술적으로 가능하긴 하지만, 두 가지 이유로 위험하다. 첫째, 모든 worker의 destroy/reinit 타이밍을 동기화하기 어렵다 — 일부 worker가 destroy를 완료하기 전에 다른 worker가 reinit을 시작하면 NCCL이 invalid state에 빠질 수 있다 ([pytorch/pytorch#119196](https://github.com/pytorch/pytorch/issues/119196), 2026-02 기준 여전히 open). 둘째, Ray Train의 [`on_shutdown`](https://github.com/ray-project/ray/blob/master/python/ray/train/torch/config.py) hook이 별도로 `destroy_process_group()`을 호출하므로, 사용자가 먼저 destroy하면 Ray 측 라이프사이클과 충돌할 수 있다. 권장하지 않는다.
