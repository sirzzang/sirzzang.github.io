...
kubeconfig의 **개념, 구조(clusters·users·contexts), 파일 위치·우선순위, 다중 클러스터 사용법, API Reference**는 별도 시리즈에서 다룬다. 필요하면 다음 글을 참고하자.

- [kubeconfig - 1. 개요]({% post_url 2026-02-16-Kubernetes-Kubeconfig-01 %}): 개념, 정의, 사용 주체(컴포넌트/사용자), 필요성, 구조, 설정 파일 위치·우선순위, 사용법
- [kubeconfig - 2. 다중 클러스터 접근 구성 실습]({% post_url 2026-02-16-Kubernetes-Kubeconfig-02 %}): 여러 클러스터를 하나의 kubeconfig로 관리하는 실습 (Multipass, kind, k3s, kubeadm)
- [kubeconfig - 3. API Reference 톺아보기]({% post_url 2026-02-16-Kubernetes-Kubeconfig-03 %}): Config, Cluster, AuthInfo, Context 등 kubeconfig 필드 스펙 상세
