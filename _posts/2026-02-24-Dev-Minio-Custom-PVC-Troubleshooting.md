...
2025년 초, 현장에 배포된 K3s 클러스터에서 MinIO를 운영하던 중 루트 파티션이 90%까지 차오르며 워크플로우가 중단되는 문제가 발생했다. 원인은 MinIO Helm chart의 `distributed` 모드에서 `persistence.existingClaim`이 **조용히 무시**되는 구조에 있었다. SSD를 증설하고, SSD 마운트 경로를 스토리지 경로로 설정한 뒤 README 안내대로 MinIO를 재배포해도 문제는 해결되지 않았다. Helm Values에 `persistence.existingClaim`을 분명히 설정했는데, 엉뚱하게도 데이터는 기존과 동일하게 K3s 기본 StorageClass가 루트 파티션에 생성한 PV에 계속 쌓이고 있었다. 결국 Helm 템플릿 코드를 직접 파고들어서야, `distributed` 모드에서 `existingClaim`이 구조적으로 무시된다는 사실을 알게 되었다.

당시에는 급하게 해결하고 넘어갔는데, 최근 리눅스 스토리지와 Kubernetes 스토리지 구조를 공부하면서 이 문제를 다시 꺼내 정리하게 되었다. 이 글에서는 그 트러블슈팅 과정과 해결, 데이터 이관에서의 삽질까지 복기한다.
...
데이터 이관 자체는 루트 파티션에서 SSD로 복사하는 작업이므로, 루트 파티션의 사용량을 늘리지는 않는다. 문제는 이관을 준비하는 과정에서, **MinIO가 아직 루트 파티션의 기존 PV에 데이터를 쓰고 있었다**는 점이다. 잔여 용량 23GB 상태에서 MinIO가 계속 데이터를 쌓으면서 kubelet의 eviction threshold(기본값: 노드 전체 용량의 약 15%)를 넘어섰고, Pod 축출이 시작되었다. MinIO뿐 아니라 같은 노드의 다른 워크로드에도 영향이 미쳤다.
...
- README와 values.yaml의 안내를 따랐는데도 `existingClaim`이 적용되지 않는 원인 추적
- `distributed` 모드(StatefulSet)에서 `existingClaim`이 구조적으로 무시되는 이유
