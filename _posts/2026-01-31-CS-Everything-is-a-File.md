...
[앞에서 살펴본 것처럼](#파일의-두-가지-의미), 파일은 여러 가지 맥락으로 사용된다. "Everything is a File" 철학은, 철학적으로는 **파일 추상화**(1번, 넓은 의미)를 지향하지만, 실제 구현에서는 아래와 같은 한계가 있다.

## 기본 파일 연산으로 부족한 경우

**기본 파일 연산**(2번, 좁은 의미)만으로는 모든 리소스를 파일로 추상화하기에 부족함이 있다. 
