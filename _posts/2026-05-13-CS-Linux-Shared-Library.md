...
같은 soname(`libnccl.so.2`)을 가진 `libnccl.so.2.26.2`와 `libnccl.so.2.29.7`이 같은 디렉토리에 있을 때 `ldconfig`를 실행하면, 두 파일을 같은 soname 그룹으로 묶은 뒤 그 그룹 안에서 **버전 문자열을 자연순(version sort)으로 비교해 큰 쪽**(`libnccl.so.2.29.7`)으로 심볼릭 링크를 만든다. 그룹화는 각 `.so`의 `DT_SONAME` 기준이므로, `libnccl.so.2.x`와 `libnccl.so.3.x`가 같은 디렉토리에 공존해도 서로 다른 그룹으로 분리되어 각자 별개의 심볼릭 링크가 만들어진다. 결과적으로 같은 major 안에서 minor/patch가 큰 쪽이 선택된다.

> 자연순 비교 자체는 glibc의 `strverscmp(3)`로도 가능하지만, ldconfig는 이를 직접 호출하지 않고 내부 비교 함수(`_dl_cache_libcmp` 등)로 동등한 자연순 비교를 자체 구현해 사용한다.
...
- **심볼 이름**: 컴파일된 바이너리에서 함수/변수가 어떤 이름으로 노출되는가 (C++ name mangling — 오버로딩·네임스페이스·템플릿 때문에 같은 이름의 함수가 여럿 존재할 수 있어, 컴파일러가 인자 타입·소속 클래스 등을 인코딩해 `_ZN3foo3barEi` 같은 고유 심볼명으로 변환하는 규칙 — 포함)
...
ABI와 API는 별개 차원이다. 한쪽이 호환이라고 해서 다른 쪽도 자동으로 호환되지는 않는다. 어떤 비대칭이 가능한지, 그리고 공유 라이브러리에서 이 약속을 어떻게 관리하는지가 다음 절의 주제다.

<details markdown="1">
<summary>참고: REST API 같은 "백엔드 API"도 같은 정의에 들어가나?</summary>

들어간다. 프런트가 백엔드 API를 호출할 때 의존하는 약속들 — `GET /api/users/{id}` 경로가 존재한다, 200 OK가 오면 `{ "id": ..., "name": ... }` 형태의 JSON이 온다, 404는 "유저 없음"을 뜻한다 — 이게 모두 "**호출하는 쪽 코드를 작성할 때 의존하는 계약**"이다. 백엔드가 어느 날 응답 JSON 키 중 `name`을 `username`으로 바꾸면 프런트 소스를 고쳐야 하는데, 이는 라이브러리 함수 시그니처가 바뀌면 호출하는 코드를 고쳐야 하는 것과 같은 구조다. 그래서 위 표의 "API 적용 범위 = REST, 라이브러리, 프로토콜 등"에 REST가 같이 들어가 있다.

다만 ABI 차원으로 내려가면 비대칭이 생긴다.

| | 라이브러리 API | REST API |
|---|---|---|
| 계약의 형태 | 함수 시그니처, 타입 | URL 경로, HTTP 메서드, JSON 스키마, 상태 코드 |
| 깨졌을 때 증상 | 재컴파일 실패 / 런타임 크래시 | HTTP 4xx/5xx, 파싱 에러 |
| 대응되는 ABI | **있음** (`.so`의 바이너리 레이아웃) | **없음** — 와이어 포맷이 그 자리를 대신함 |

**REST API에는 ABI 차원이 따로 없다.** 네이티브 바이너리끼리 메모리 레이아웃을 공유하는 게 아니라 HTTP 위의 텍스트(혹은 protobuf 같은 정의된 와이어 포맷)로 통신하기 때문에, "컴파일 결과물 수준의 호환성"이라는 개념 자체가 성립하지 않는다. 위 표에서 ABI 적용 범위가 "네이티브 바이너리 간 인터페이스에 한정"으로 좁혀져 있는 이유가 이것이다 — REST API는 큰 우산인 API 안에는 들어오지만, ABI 우산 밖에 있다.

네트워크 프로토콜 세계에서 "ABI 비슷한 호환성"을 흉내내려는 장치는 따로 있다. 예: protobuf의 "field number를 재사용하지 마라" 규칙, GraphQL의 `@deprecated` 정책, REST의 `/v1/`, `/v2/` 같은 URL 버저닝. 이들은 와이어 포맷 수준에서 "기존 클라이언트를 깨뜨리지 않는다"는 약속을 만든다는 점에서 soname의 ABI 보장과 역할이 비슷하다.

</details>

## soname이 보장하는 계약

공유 라이브러리에서 soname이 일차적으로 보장하려는 것은 바이너리 쪽(ABI)이다. "재컴파일 없이 `.so` 교체만으로 동작"을 약속하는 게 soname의 핵심이기 때문이다. 이 약속은 라이브러리 제공자가 일방적으로 선언한다고 성립하는 게 아니라, 제공자와 소비자가 양쪽에서 같이 지켜야 의미가 있는 계약이다. 어떤 약속인지 양쪽 입장에서 풀어 보면 다음과 같다.
...
같은 메이저 안에서 ABI를 깨지 않으려면 라이브러리 쪽이 다음 관례를 따른다:

- **함수 시그니처는 유지**: 인자 타입·개수·반환 타입을 바꾸지 않는다. 새 동작이 필요하면 기존 함수를 그대로 두고 새 함수를 추가한다
- **struct 필드는 끝에만 추가**: 기존 필드의 오프셋이 변하지 않는다. 가운데에 끼워넣거나 순서를 바꾸면 옛 바이너리가 옛 오프셋으로 메모리를 잘못 읽게 된다
- **opaque struct 활용**: 헤더에서는 forward declaration(`typedef struct ncclComm* ncclComm_t`)으로만 노출하고 정의는 라이브러리 내부에 숨기면, 소비자 코드가 필드에 직접 접근할 수 없어 layout 변경이 자유로워진다

이 관례를 지키는 한, 2.26.2 → 2.29.7 minor 업그레이드는 소비자 입장에서 `.so` 파일만 교체하면 끝이다.

### 약속을 어기는 변경 — API 호환 / ABI 비호환의 발현

가장 자주 만나는 비대칭 사례를 사고 실험으로 보자. NCCL이 같은 메이저(`.so.2`)를 유지한 채 `struct ncclComm`의 기존 필드 사이에 새 필드를 끼워넣었다고 하자. 이건 위 관례를 위반하는 변경이지만, ABI 비호환이 어떻게 발현되는지 보여주는 대표적인 예다.

- **왜 ABI 비호환인가**: C/C++의 struct 필드 접근(`comm->rank`)은 컴파일 시점에 **숫자 오프셋으로 변환**되어 바이너리에 박힌다 — "rank라는 이름"이 아니라 "베이스 + N바이트". NCCL을 사용하는 쪽(예: PyTorch)이 옛 헤더로 빌드해 둔 바이너리는 옛 N을 그대로 들고 있으니, 새 `.so`로 교체하면 그 위치에 새로 끼워넣은 필드 값이 놓여 있어 잘못된 값을 읽거나 크래시한다
- **왜 API 호환인가**: 함수 호출(`ncclCommInit(...)`)은 오프셋이 아니라 심볼명으로 link되므로, 시그니처가 같으면 옛 바이너리도 새 `.so`의 함수를 그대로 호출할 수 있다. 즉 헤더(=API)의 함수 시그니처 자체는 변하지 않았다
- **소비자가 다시 컴파일하면 왜 동작하나**: 컴파일러가 새 헤더의 struct 정의를 다시 읽고 새 오프셋(N')을 코드에 다시 박아주기 때문이다. 헤더가 곧 API이므로, "새 헤더로 다시 컴파일한다 = 새 API에 맞춰 새 오프셋이 박힌다"이다. 결과적으로 소스 변경은 필요 없지만 **재빌드는 필요**하다 — 이게 "API는 호환이지만 ABI는 비호환"의 정확한 의미다

따라서 이런 변경이 진짜로 필요하면 정상 운영 관례는 **soname을 `.so.3`으로 올리는 것**이다 (= "옛 .so.2와는 호환 안 함" 신호). soname 자체는 강제 검사 장치가 아니므로, 같은 메이저를 유지한 채 이런 변경을 배포하면 패키지 매니저는 막아주지 못한다 — 이건 라이브러리 저자의 책임 영역이다.

<details markdown="1">
<summary>반대 방향: API 비호환이지만 ABI 호환인 경우</summary>

<br>
반대의 경우도 드물지만 가능하긴 하다. 예를 들어:
...
soname의 보장에는 분명한 한계가 있다. 이름표가 강제 검사 장치가 아니라는 점, 그리고 ABI 호환이 곧 동작 동일성을 의미하지 않는다는 점 두 가지다.

### 동적 링커는 ABI 자체를 검사하지 않는다

동적 링커가 하는 일은 ABI 내용을 검증하는 게 아니라, soname을 이름으로 분리해서 비호환 로드를 사실상 차단하는 것이다. 만약 soname 관례 없이 `libnccl.so`만 사용했다면, 라이브러리 업그레이드 시 ABI 비호환인 새 버전이 아무 경고 없이 로드되어 segfault나 데이터 손상으로 이어질 수 있다. soname은 이런 의도치 않은 비호환 로드를 구조적으로 차단하는 설계다.
...
`.so` 파일에는 "나를 로드하려면 이 `.so`들도 함께 로드해야 한다"는 의존성 목록이 기록되어 있다 — ELF Dynamic Section의 `DT_NEEDED` 태그다. `readelf -d <library.so> | grep NEEDED`로 확인할 수 있고, 동적 링커는 라이브러리를 로드할 때 이 목록을 따라 의존하는 `.so`들을 함께 끌어온다.

반대로 어떤 의존이 NEEDED에 **보이지 않는다**고 해서 그 라이브러리를 사용하지 않는다는 뜻은 아니다. 일반적으로 부재 가능 이유는 세 가지다:

- (a) **정적 링크**: `.a`(정적 라이브러리)로 빌드 시점에 코드 자체를 바이너리 안에 복사한 경우. 외부 `.so`를 탐색할 필요가 없으므로 NEEDED에도 안 남는다
- (b) **`dlopen` 지연 로드**: 런타임에 명시적으로 `dlopen("libfoo.so")`을 호출하는 경우. 의존 라이브러리 이름이 코드에 문자열로 박혀 있을 뿐 ELF 헤더의 NEEDED에는 들어가지 않는다
- (c) **실제로 의존하지 않음**

따라서 "NEEDED 부재 = 정적 링크"로 단정해서는 안 되고, `nm -D`로 동적 심볼 종류(`T`/`W` 정의됨, `U` 미정의)를 보거나 `strings`로 라이브러리 이름이 박혀 있는지 등의 positive evidence와 함께 판단하는 편이 안전하다.
...
정적 링크된 코드 자체는 soname 체계와 무관하다 — 빌드 시 `.a`의 코드가 바이너리 안에 복사되므로, 그 부분은 런타임에 외부 `.so`를 탐색할 일이 없다. 단, 정적 링크를 포함한 바이너리도 다른 동적 라이브러리(libc, libpthread 등)에는 여전히 의존할 수 있고, 그쪽은 NEEDED에 남는다.
...
[같은 soname의 복수 존재](#같은-soname의-복수-존재)에서 정리한 "배포 채널이 다르면 같은 soname의 `.so`가 각 채널 경로에 별개로 존재할 수 있다"는 일반 구조는, GPU/CUDA 영역에서 특히 두드러진다. apt 채널과 pip 채널 양쪽 모두에서 같은 NVIDIA 라이브러리를 받을 수 있게 되면서, 한 시스템 안에 두 경로가 동시에 채워지는 일이 일상이 됐기 때문이다. 두 채널의 설치 경로를 NVIDIA 라이브러리 기준으로 정리하면 다음과 같다.
...
`site-packages`에 `.so`가 있는 것 자체는 새로운 일이 아니다 — Python C 확장은 원래 `.so`로 컴파일된다. NVIDIA가 한 것은 기존에 apt/yum으로만 설치하던 **독립 시스템 라이브러리**(libnccl, libcublas, libcudart 등)를 pip 패키지로도 배포하기 시작한 것이다 (2022~2023년경 공식화). `pip install torch`만으로 CUDA 런타임까지 전부 딸려오는 "userspace-level self-contained GPU 환경"을 만들기 위한 전략이었고(호스트 GPU 드라이버는 여전히 별도), 그 결과 앞 절에서 일반론으로 정리한 이중 구조가 GPU 라이브러리 전반에 걸쳐 굳어졌다.
...
앞 절의 일반 원리(NEEDED 부재의 세 가지 가능성)를 NCCL `.so`에 직접 적용해 보자.
...
NEEDED는 모두 시스템 표준 라이브러리(C 런타임, pthread, 동적 링커 자체)다. NCCL은 GPU 통신 라이브러리이므로 당연히 CUDA에 의존하지만, **NEEDED에는 `libcudart.so` 같은 CUDA runtime이 한 줄도 보이지 않는다**. (a)~(c)에 비추어 NCCL의 경우 가능한 시나리오는:
...
NCCL 공식 빌드 가이드의 기본값이 `cudart_static`이라 (a)가 가장 흔하지만, NEEDED 부재 한 가지로 단정하기보다는 빌드 매뉴얼이나 `strings libnccl.so.2 | grep cudart` 같은 positive evidence로 확인하는 편이 안전하다. 더 직접적으로는 `nm -D libnccl.so.2 | grep -i cuda`로 동적 심볼 종류를 보는 방법이 있다. NEEDED에 `libcudart.so`가 없는데 `cudart_*` 심볼이 `T`/`W`(정의됨)로 잡히면 정적 링크의 강한 증거이고, `U`(undefined)로 잡히는데도 NEEDED에 보이지 않는다면 `dlopen` 경로일 가능성이 크다.

NCCL pip 번들도 일반적으로 CUDA runtime은 정적, glibc 계열은 동적으로 남는 구성이 흔하다. 본인 환경에서는 `readelf -d $(python -c "import nvidia.nccl, os; print(os.path.dirname(nvidia.nccl.__file__) + '/lib/libnccl.so.2')")` 같은 식으로 직접 확인하는 편이 안전하다.

## 정적 링크의 동기와 대가

NCCL이 CUDA runtime을 정적으로 머금는 결정의 동기는 pip 배포 모델과의 정합성이다. pip으로 배포할 때는 대상 시스템에 어떤 CUDA가 설치되어 있는지 보장할 수 없다 (`pip install torch`를 실행하는 환경에 `libcudart.so`가 아예 없을 수도 있음). 정적 링크하면 GPU 가속 라이브러리 측면에서는 "`.so` 하나만 있으면 동작한다"에 가까운 자급력을 얻는다 (호스트 GPU 드라이버(`libcuda.so.1`)에는 여전히 의존하므로 엄밀히 self-contained 바이너리는 아니다).
