...
어플리케이션 설정이 코드(이미지)에 묻혀 있으면 세 가지 문제가 생긴다.

1. **환경별 이미지 분리**: 개발/스테이징/프로덕션마다 별도 이미지를 빌드해야 한다
2. **재빌드 사이클**: 설정 한 줄 바꾸는 데 이미지 재빌드 → 재배포 파이프라인을 돌려야 한다
3. **민감 정보 잔류**: 이미지 레이어에 비밀번호나 API 키가 남을 수 있다

Kubernetes는 이 문제를 **ConfigMap**과 **Secret**이라는 두 API 오브젝트로 해결한다. 설정 데이터를 별도 오브젝트에 저장하고, 파드가 이를 참조하게 하면 이미지와 설정을 완전히 분리할 수 있다.
...
**ConfigMap**은 비기밀(non-confidential) 키-값 쌍을 저장하는 Kubernetes API 오브젝트다. ([Kubernetes 공식 문서: ConfigMaps](https://kubernetes.io/docs/concepts/configuration/configmap/))

- 값은 짧은 문자열부터 설정 파일 수준의 긴 구조화된 텍스트까지 가능하다
- 파드는 ConfigMap의 키-값 항목을 환경 변수, 커맨드라인 인자, 또는 볼륨 내 설정 파일로 사용할 수 있다
...
ConfigMap 생성 방법은 상황에 따라 선택한다.

- **빠른 테스트**: `kubectl create configmap` — CLI에서 바로 생성
- **선언적 관리**: YAML 매니페스트 — Git에 커밋하여 버전 관리
- **CI/CD 파이프라인**: `--dry-run=client -o yaml` — 매니페스트를 생성만 하고, GitOps 워크플로우에 태움
...
ConfigMap의 값이 변경되는 것을 방지하려면, `immutable: true`로 설정한다. Kubernetes v1.21부터 지원되며, 불변 ConfigMap은 API 서버가 변경 감시(watch)를 유지할 필요가 없어 대규모 클러스터에서 API 서버 부하를 줄여주는 효과도 있다. ([Kubernetes 공식 문서: Immutable ConfigMaps](https://kubernetes.io/docs/concepts/configuration/configmap/#configmap-immutable))
...
# 실무 관점: ConfigMap 관리 도구

실제 운영 환경에서는 ConfigMap을 수동으로 관리하기보다 도구를 사용하는 경우가 많다.

- **Kustomize `configMapGenerator`**: Kustomize는 ConfigMap 이름에 콘텐츠 해시 suffix를 자동 추가한다. ConfigMap 내용이 바뀌면 이름도 바뀌므로, Deployment가 이를 참조하고 있으면 자동으로 새 Pod가 롤아웃된다. 환경 변수 방식의 "재시작 전까지 반영 안 됨" 문제를 우회하는 패턴이다.

- **Helm `values.yaml`**: Helm 차트에서는 `values.yaml`에 설정을 정의하고, 템플릿이 이를 ConfigMap으로 렌더링한다. 환경별로 `values-dev.yaml`, `values-prod.yaml`을 분리하면 하나의 차트로 여러 환경에 배포할 수 있다.

| 도구 | ConfigMap 변경 시 롤아웃 | 환경별 분리 | Git 관리 |
| --- | --- | --- | --- |
| 수동 (`kubectl apply`) | X (별도 재시작 필요) | 매니페스트 파일 분리 | 가능하지만 번거로움 |
| Kustomize | O (해시 suffix 자동) | overlay 디렉토리 | 네이티브 지원 |
| Helm | O (차트 릴리즈 업그레이드) | values 파일 분리 | 네이티브 지원 |
