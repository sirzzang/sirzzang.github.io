...
`nodeStatusMaxImages`의 기본값이 50이므로, 노드에 50개가 넘는 이미지가 있으면 `kubectl`로 조회할 때 일부가 누락된다. ML/AI 워크로드처럼 학습 이미지가 많은 환경에서는 시스템 이미지까지 합치면 50을 쉽게 초과할 수 있다. 정확한 전수 조회가 필요하면 이 값을 `-1`(무제한) 또는 충분히 큰 값으로 변경해야 한다.
