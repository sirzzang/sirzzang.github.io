---
title:  "[Kubernetes] Cluster: Kubespray를 이용해 클러스터 구성하기 - 4-0. 프로젝트 구조 Overview"
excerpt: "Kubespray 프로젝트의 전체 디렉토리 구조를 살펴보고, 핵심 파일들을 파악한다."
categories:
  - Kubernetes
toc: true
header:
...
> 이전 글에서 [Kubespray로 맛보기 클러스터를 구성]({% post_url 2026-01-25-Kubernetes-Kubespray-02 %})해 보았다. 이번 글부터는 Kubespray 내부 구조를 본격적으로 분석한다.
...
Kubespray의 Ansible 설정 파일이다. **Kubespray 디렉토리에서 실행해야** 이 설정이 적용된다.
...
- **ControlMaster=auto**: SSH 연결 재사용 (멀티플렉싱)
- **ControlPersist=30m**: 마스터 연결 30분 유지
- **ConnectionAttempts=100**: 연결 재시도 횟수
- **UserKnownHostsFile=/dev/null**: known_hosts 무시

이 설정들은 **수십~수백 대 노드에 반복 접속**하는 Kubespray 특성에 맞게 최적화되어 있다.
