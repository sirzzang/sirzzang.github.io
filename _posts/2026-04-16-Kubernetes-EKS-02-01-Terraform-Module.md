...
[이전 포스트]({% post_url 2026-04-16-Kubernetes-EKS-02-00-02-Cluster-Flux-Architecture %})에서 Flux 아키텍처를 분석하면서, Git 저장소에 있는 `tenant-apps` Terraform 모듈이 Tofu Controller를 통해 자동 실행되는 구조를 확인했다. 이번 포스트에서는 GitOps 자동화로 넘어가기 전에, 이 모듈이 실제로 어떤 리소스를 만드는지 수동으로 검증한다. `terraform plan`까지만 수행하고, `terraform apply`는 하지 않는다.
