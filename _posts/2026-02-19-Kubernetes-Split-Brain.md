...
quorum을 **과반(절반 초과)**으로 두기 때문에, 한 번에 하나의 파티션만 quorum을 가질 수 있어 네트워크 파티션만으로는 "양쪽 모두 쓰기"가 불가능하다. Raft가 정확히 이 문제를 방지하기 위해 설계된 알고리즘이며, 그래서 etcd/Kubernetes 컨트롤 플레인도 홀수 대수 구성을 권장하는 것이다.
...
etcd 멤버 2개, 장애 없이 유지되고 있다. 다만, 이 상태에서는 1개만 장애가 나도 quorum을 상실하여 클러스터 전체에 문제가 생기므로, 빠르게 삼중화를 복구해야 한다.
...
재조인 전에 `member list`, `endpoint health` 등으로 etcd 상태를 확인해야 한다. 비정상일 때의 대표 시나리오와 대응을 정리한다. (아래 “Split Brain + 기존 클러스터 노드 하나가 죽어 있었다면”은 **시나리오 2 + Split Brain이 겹친** 복합 케이스다.)

### 시나리오 1: 멤버 3개 중 1개 unhealthy

`member list`에는 3명이 보이지만, `endpoint health`에서 한 멤버만 `false`인 경우다. quorum(2/3)은 유지되므로 **쓰기는 가능**하다. 대응: 더 이상 복구할 수 없다고 판단되면 해당 멤버를 etcd 클러스터에서 제거한다.

```bash
# 기존 컨트롤 플레인(cp-node-a)에서 실행. unhealthy 멤버의 ID 사용
$ sudo ETCDCTL_API=3 etcdctl \
  --endpoints=https://127.0.0.1:2379 \
  --cacert=/var/lib/rancher/k3s/server/tls/etcd/server-ca.crt \
  --cert=/var/lib/rancher/k3s/server/tls/etcd/server-client.crt \
  --key=/var/lib/rancher/k3s/server/tls/etcd/server-client.key \
  member remove <unhealthy멤버_ID>
```

제거 후에는 해당 노드에서 K3s 데이터를 정리한 뒤 `--server`를 지정해 재조인한다.

### 시나리오 2: quorum 상실 (2멤버 중 1개만 살아 있음)

한 노드(cp-node-b)가 죽어 있고, 다른 한 노드(cp-node-a)만 살아 있는 상황이다. 2노드에서는 quorum이 2이므로 1/2로 **quorum 미달** → 클러스터가 읽기 전용이거나 동작 불가가 된다.
...
1. **죽은 노드 복구 시도** (우선)
   ...
   $ sudo reboot
   ```
2. **응급 조치** (복구 불가 시): 살아 있는 노드에서 단일 노드로 재시작. **권장되지는 않으며**, 다른 노드의 etcd 데이터는 더 이상 사용하지 못하게 된다.
   ```bash
   # cp-node-a에서 (살아 있는 노드)
   $ sudo k3s server --cluster-reset
   # 경고: etcd 클러스터가 단일 노드로 재초기화됨. 다른 노드는 데이터 정리 후 재조인 필요.
   ```

**Split Brain까지 겹친 경우**(예: cp-node-c는 독립 클러스터로 떨어져 있고, cp-node-b는 죽어 있음)는 아래 “Split Brain + 기존 클러스터 노드 하나가 죽어 있었다면”을 참고한다.

### 시나리오 3: Split Brain

한 노드가 기존 클러스터와 별개의 etcd 클러스터로 부팅해, 원래 클러스터에는 2개 멤버만 보이고, 분리된 노드는 독립 클러스터로 동작하는 상황이다. **대응은 본문 “해결” 섹션**과 동일하다. 원래 클러스터에서 해당 노드 제거 → 분리된 노드에서 K3s 완전 제거 → `--server` 지정 후 재조인.

### 시나리오 4: 멤버는 모두 healthy인데 데이터 불일치 (member ID mismatch)

`member list`는 정상인데 로그에 `etcd member ID mismatch` 등이 나오는 경우다. **대응:** etcd 데이터 정합성을 확인한 뒤, 필요하면 스냅샷 복구를 검토한다.
...
모든 엔드포인트의 **revision**이 동일한지 확인한다. 불일치가 있으면 K3s/etcd 문서의 스냅샷 복원 절차에 따라 정리한 뒤 재조인을 진행한다.


## Split Brain + 기존 클러스터 노드 하나가 죽어 있었다면
...
이때 **기존 클러스터(cp-node-a + cp-node-b) 쪽 etcd는 이미 quorum을 잃은 상태**다. cp-node-c가 독립 클러스터로 분리되었더라도, 원래 클러스터의 etcd member list에서는 자동으로 제거되지 않는다. 원래 클러스터 입장에서는 여전히 cp-node-a, cp-node-b, cp-node-c 세 멤버가 등록되어 있고, 그 중 cp-node-b와 cp-node-c가 응답하지 않는 상황이다. 3멤버 중 1개(cp-node-a)만 살아 있으므로 1/3으로 **quorum 미달** → 클러스터가 읽기 전용이거나 동작 불가가 된다.

**우선순위는 반드시 다음 순서다.**

1. **먼저: 기존 클러스터의 quorum 복구**
   - cp-node-b 복구 시도: `systemctl restart k3s` 또는 재부팅
   - cp-node-b 복구가 불가능하면: cp-node-a에서 `k3s server --cluster-reset`으로 단일 노드로 재시작하는 **응급 조치**를 검토
   - **이유**: quorum이 없으면 etcd에 쓰기가 불가능하므로, Split Brain 노드를 멤버에서 제거하는 작업조차 할 수 없다.
2. **그 다음: Split Brain 노드 대응**
   - quorum이 확보된 뒤에야 `etcdctl member remove`로 cp-node-c를 제거할 수 있다.
...
3. **재조인 전 etcd 상태를 확인한다.** etcd 멤버 목록과 헬스 상태를 점검한 뒤, 문제가 있으면 해당 시나리오에 맞게 대응한 후에 재조인을 진행한다.
