---
title:  "[Kubernetes] Service와 kube-proxy"
excerpt: "쿠버네티스 Service 추상화, kube-proxy의 동작 모드, 그리고 ClusterIP·NodePort·LoadBalancer 패킷 흐름에 대해 알아 보자."
categories:
  - Kubernetes
toc: true
header:
...
- kube-proxy 모드: **userspace**(초기, 프로세스가 직접 프록시) → **iptables**(기본, 커널 DNAT) → **IPVS**(해시 O(1), v1.35에서 deprecated) → **nftables**(iptables 후속) → **eBPF**(커널 스택 우회, Cilium)

<br>

# 들어가며

[파드 간 통신]({% post_url 2026-05-04-Kubernetes-Networking-01-Pod-to-Pod %}) 글에서 쿠버네티스 네트워킹의 핵심인 **문제 2 — 파드 간 통신**을 다뤘다. 같은 노드에서는 veth pair + 브릿지로, 다른 노드에서는 오버레이/BGP/클라우드 네이티브 라우팅으로 NAT 없이 파드 간 직접 통신을 구현하는 방식이었다.

이 글에서는 [네트워킹 모델]({% post_url 2026-05-04-Kubernetes-Networking-00-Model %}#문제-3-파드--서비스)에서 간단히 언급했던 **문제 3 — 파드 ↔ 서비스**, 그리고 **문제 4 — 외부 ↔ 서비스** 통신을 본격적으로 다룬다.

여기서 다루는 Service 계층은, 앞선 글들에서 다뤘던 인프라 계층과 성격이 다르다. 컨테이너 네트워킹(같은 노드 내 파드 간 통신)과 CNI(다른 노드 파드 간 통신)가 **"파드 IP로 직접 통신할 수 있는 flat network"를 만드는 인프라**라면, Service는 그 인프라 위에 얹히는 **추상화 계층**이다. 아래에서는 이 추상화가 무엇을 해결하고, 어떻게 동작하며, 왜 가상 IP라는 형태를 취하는지를 살펴본다.
...
[네트워킹 모델]({% post_url 2026-05-04-Kubernetes-Networking-00-Model %}#쿠버네티스-네트워킹의-4가지-문제) 글의 문제 분류와 연결해, 각 서비스 타입이 4가지 네트워킹 문제 중 어느 문제를 해결하는지 확인해 보자.
...
CNI 플러그인(여기서는 [AWS VPC CNI]({% post_url 2026-03-19-Kubernetes-EKS-02-01-01-EKS-VPC-CNI %})를 예시로)도 POSTROUTING에 규칙을 설치한다. 목적은 kube-proxy와 완전히 다르다.
...
여기서 의문이 생길 수 있다. 쿠버네티스 네트워킹 모델은 ["NAT 없이"]({% post_url 2026-05-04-Kubernetes-Networking-00-Model %}#핵심-원칙-nat-없이)를 대원칙으로 내세우는데, kube-proxy의 DNAT과 MASQUERADE, CNI의 SNAT은 전부 NAT이 아닌가?

[네트워킹 모델]({% post_url 2026-05-04-Kubernetes-Networking-00-Model %}#원칙의-적용-범위) 글에서 정리했듯이, **위배되지 않는다**. "NAT 없이" 원칙은 **문제 2(파드 ↔ 파드 직접 통신 경로)**에만 적용된다.
...
커널 netfilter 서브시스템의 **iptables API**로 패킷 포워딩 규칙을 관리하는 모드다. v1.2부터 기본값이 되었고, 대부분의 쿠버네티스 클러스터에서 현재도 **기본값**으로 사용된다.
...
> **IPVS 모드 deprecated (v1.35)**: [Kubernetes v1.35 (Timbernetes, 2025-12-17 릴리스)](https://kubernetes.io/blog/2025/12/17/kubernetes-v1-35-release/)에서 kube-proxy의 IPVS 모드가 [deprecated되었다](https://github.com/kubernetes/enhancements/issues/5495) (KEP-5495). IPVS와 다른 kube-proxy 모드 사이의 기능 동등성(feature parity)을 유지하는 비용이 커져 기술 부채가 누적되었기 때문이며, 추후 릴리스에서 실제로 제거될 예정이다. Linux 노드에서 권장되는 후속 모드는 **nftables**다.
...
[Cilium의 kube-proxy replacement](https://docs.cilium.io/en/stable/network/kubernetes/kubeproxy-free/)는 커널 **4.19+**가 최소 요구치이며 (`socket-LB` 등 일부 기능은 5.7+ 필요), 최신 Cilium 버전을 풀 기능으로 쓰려면 5.10 이상이 권장된다. 정확한 매트릭스는 사용 중인 Cilium 버전의 [System Requirements](https://docs.cilium.io/en/stable/operations/system-requirements/) 문서를 참고한다. kube-proxy를 아예 배포하지 않고(`--kube-proxy-replacement=true`) Cilium이 Service 구현을 전부 담당한다.
...
| **요구사항** | — | 기본 내장 | `ip_vs` 모듈 필요 | 커널 5.13+ | Cilium CNI + 커널 4.19+ (최신 기능은 5.10+ 권장) |
| **상태** | **v1.0 기본 → 폐기** | 현재 기본값 | **v1.35에서 deprecated** | beta (v1.31+) | Cilium 전용 |
...
대부분의 매니지드 쿠버네티스 환경(EKS, GKE, AKS 등)은 현재 **iptables**가 기본값이다. 서비스가 수천 개 이상이면 nftables 전환을 고려하고, 고성능 + L7 정책이 필요하면 Cilium eBPF를 검토한다. v1.35의 IPVS deprecation 이후로는 신규 배포에서 IPVS를 선택할 이유가 거의 없다.
...
> IP Target Mode는 [VPC CNI]({% post_url 2026-03-19-Kubernetes-EKS-02-01-01-EKS-VPC-CNI %})의 "파드 IP = VPC IP" 특성을 활용한다. 이 구조의 세부 동작 — `externalTrafficPolicy` 설정, Pod Readiness Gate, 헬스 체크 등 — 은 EKS 서비스 실습에서 구체적으로 다룬다.
...
- **kube-proxy 모드**: userspace(초기, SPOF) → iptables(기본, 커널 DNAT) → IPVS(해시, **v1.35에서 deprecated**) → nftables(후속) → eBPF(스택 우회)
- **패킷 흐름**:
...
- [Kubernetes v1.35: Timbernetes 릴리스 블로그](https://kubernetes.io/blog/2025/12/17/kubernetes-v1-35-release/) — IPVS deprecation 공지
- [KEP-5495: Deprecate IPVS mode in kube-proxy](https://github.com/kubernetes/enhancements/issues/5495)
- [Cilium kube-proxy replacement 문서](https://docs.cilium.io/en/stable/network/kubernetes/kubeproxy-free/)
