...
Canal의 `iface: "enp0s9"` 설정은 Flannel이 [VXLAN]({% post_url 2026-03-19-Kubernetes-Networking-03-CNI-Flow %}) [오버레이 네트워크]({% post_url 2026-03-19-Kubernetes-Networking-03-CNI-Flow %})로 캡슐화한 패킷을 실제로 내보낼 물리 인터페이스를 지정한다. VirtualBox 환경에서는 NIC가 여러 개(NAT용 `enp0s3`, Host-Only용 `enp0s9`)이므로, 명시하지 않으면 Flannel이 기본 게이트웨이가 있는 NAT 인터페이스를 선택하여 노드 간 통신이 실패한다. 이 역시 두 번째 NIC를 사용하도록 맞추는 설정이다.
