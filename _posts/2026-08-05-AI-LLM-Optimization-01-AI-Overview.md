...
앞서 뉴런 하나는 판단 기준(질문) 하나밖에 못 던진다고 했는데, 이렇게 여러 뉴런을 나란히 두면 바로 그 한계가 보완된다. 같은 입력을 받고도 뉴런마다 서로 다른 **특징**(*특성, feature, 피처 등으로도 불린다. 이하 특징으로 통칭*)을 나눠 읽어, 하나로는 놓쳤을 여러 각도의 판단 기준을 동시에 확보하는 것이다. 실제 이미지 인식 신경망에서도 같은 그림이 나타난다. 아래는 같은 입력(고양이 사진, 손글씨 숫자 2)을 받은 뉴런들이 윤곽·눈·귀 같은 서로 다른 특징에 반응하며 역할을 나눠 갖는 모습이다.
...
이때 행렬의 **행은 뉴런 하나**이고, **열은 입력 정보 하나**다. 즉 **"여러 개의 판단 기준을 같은 입력에 동시에 적용한다"는 행위가 곧 행렬 곱이다.** 일차식 세 줄을 세로로 쌓아 행렬 하나로 묶었으니, **한 층의 계산은 그 자체로 행렬 곱 한 번**이다.
...
여기서 헷갈리기 쉬운 지점을 하나 짚고 가자. 데이터를 다룰 때는 흔히 "행 = 데이터(샘플) 개수, 열 = 특징 개수"로 배운다. 이 독법은 **데이터 행렬** $X$를 해석할 때는 유효하다. 그러나 지금 보고 있는 것은 **가중치 행렬** $W$다. 서로 다른 행렬이므로 행과 열의 의미도 다르다.

- 데이터 행렬 $X$: 행 = 샘플 개수, 열 = 입력 특징 개수
- 가중치 행렬 $W$: 행 = 뉴런 개수(= 출력 차원), 열 = 입력 차원

사실 $W$도 양쪽 축이 모두 특징을 센다. 행은 이 층이 새로 만드는 출력 특징의 개수이고, 열은 앞에서 들어오는 입력 특징의 개수다. "열 = 특징 개수"라는 익숙한 독법이 틀리는 게 아니라, 어느 표현의 특징을 세느냐가 행렬마다 다를 뿐이다.

두 행렬은 곱에서 만난다. 샘플 100개짜리 데이터 $X$(`100×2` 행렬)를 위의 뉴런 3개짜리 층(`3×2` 행렬)에 통과시키면 출력은 `100×3` 행렬이 된다(샘플마다 $z = Wx + b$를 적용한 것이고, 배치로 묶어 쓰면 $XW^\top$ 꼴이 된다 — 전치가 붙는 것은 표기 관례의 차이일 뿐이다). 행(샘플 100개)은 그대로인데 열이 2에서 3으로 바뀌었다. 새로 생긴 세 개의 열이 뉴런 세 개가 각각 뽑아낸 **새 특징**이다. 즉 한 층의 뉴런 수는 입력 특징 개수가 아니라 그 층이 새로 만들어 내는 **출력 특징 개수**이고, 이 특징들을 묶은 출력 벡터가 층이 입력을 다시 쓴 새로운 **표현(representation)**이다. 이 표현이 다음 층에서는 다시 입력 특징이 된다. 앞의 너비 절에서 "입력을 다시 표현한 새로운 특징 벡터"라고 했던 것을, 행렬의 모양과 용어로 확인할 수 있는 것이다.

> *참고*: 표현(representation)과 해석(interpretation)
>
> 층의 출력은 "뉴런들이 입력에 대해 내린 해석"이라 부르고 싶어진다. 뉴런을 판단 기준(질문)으로 본 이 글의 흐름에서는 층의 출력이 질문들에 대한 답의 묶음이니, 해석이라는 말이 자연스럽게 떠오르는 것이다. 그 직관 자체는 유효하다. 그럼에도 AI에서 이것을 **표현(representation)**이라 부르는 이유는, 층이 하는 일의 본질이 의미 부여가 아니라 **다시 쓰기**이기 때문이다. 키·몸무게라는 좌표로 적혀 있던 입력을 다른 특징 조합의 좌표로 옮겨 적을 뿐, 대상은 그대로고 적는 방식만 바뀐다. 해석이라는 말은 ML에서 모델의 판단 근거를 사람이 이해하려는 별개의 분야(해석 가능성, interpretability)에 이미 쓰이고 있기도 하다.
>
> 그러니 앞으로 AI 문헌에서 representation이라는 용어를 만나면 "모델이 입력을 자기 좋을 대로 다시 쓴 숫자 벡터" 정도로 받아들이면 충분하다. 딥러닝 자체를 **표현 학습(representation learning)**이라 부르는 것도 이 관점에서다. 다만 이렇게 만들어진 표현을 사람이 얼마나 알아볼 수 있는지는 별개의 문제인데, 이 이야기는 뒤의 "각 층의 입력과 출력"에서 이어서 짚는다.
...
여기서 **활성화 함수(activation function)**가 등장한다. 뉴런 절에서 "합이 역치를 넘으면 신호를 내보낸다"고 했던 그 역치 비교가 바로 활성화 함수의 자리다. ReLU는 음수를 0으로 눌러 버리는 아주 단순한 함수인데, "합이 0을 못 넘으면 침묵한다"는 역치 발화를 함수로 옮긴 것이다(편향 $b$가 역치를 옮겨 놓았으므로 비교점은 0이면 충분하다). 그리고 이 단순한 꺾임이 공간을 **접는다**. 늘리고 돌린 다음 한 번 접고, 다시 늘리고 돌린 다음 또 접는다. 이 "늘리기 → 접기"를 반복하면 직선이었던 경계가 임의로 복잡한 곡면이 된다. **깊이(depth)가 의미를 갖는 이유는 층을 쌓았기 때문이 아니라, 층 사이에 접는 행위가 끼어 있기 때문이다.** 그래서 활성화 함수는 있으나 마나 한 장식이 아니다. 선형변환(행렬)은 아무리 쌓아도 직선에 머물기에, 신경망에 비선형성(휘어짐)을 만들어 넣을 수 있는 것은 오직 활성화 함수뿐이다 — 딥러닝이 복잡한 판단 경계를 그릴 수 있는 힘이 바로 여기서 나온다.
...
여기서 근사의 대상이 되는 "복잡한 함수"란, 입력을 넣으면 옳은 판단이 나오는 함수다. 키·몸무게를 넣으면 "어른/아이"가 튀어나오는, 우리가 흉내 내려는 **판단 그 자체** 말이다. 글 앞에서 "어른인지 가르는 기준을 규칙으로는 못 적는다"고 했는데, 공식으로 적지 못할 뿐 그런 판단 함수는 존재하고, 신경망이 데이터로부터 그것을 근사한다. 다만 보편 근사 정리가 보장하는 것은 "그런 판단을 해내는 가중치가 **존재한다**"는 것, 즉 나타낼 수 있는 능력(representability)까지다. 학습(gradient descent)으로 그 가중치에 **반드시 도달한다**는 것까지는 보장하지 않는다. 즉 나타낼 수 있다는 것과 실제로 찾아낸다는 것은 다른 문제이고, 그 "찾아내는" 과정이 앞서 본 학습이다.

## 각 층의 입력과 출력

층을 쌓기 시작했으니, 이제 각 층의 입력과 출력이 무엇인지 짚어 둘 차례다. 규칙은 단 하나다. **어떤 층의 입력 차원은 바로 앞 층의 출력 차원이다.** 앞 층에 뉴런이 3개면 그 출력은 3차원 벡터이고, 그것이 그대로 다음 층의 입력이 된다. 이 연결만 지켜지면 각 층에 뉴런을 몇 개 둘지 — 곧 그 층의 출력 차원 — 는 설계자의 자유다. 층을 지날 때마다 특징 개수는 늘어날 수도, 줄어들 수도 있다.

다만 양 끝의 차원에는 자유가 없다.

- **최초 입력 차원**은 데이터가 정한다. 키·몸무게라면 2, 28×28 흑백 이미지라면 784다.
- **최종 출력 차원**은 풀려는 문제가 정한다. 어른/아이 같은 이진 분류면 1, 클래스 $K$개짜리 분류면 $K$, 실수 하나를 예측하는 회귀면 1이다.

입력층과 출력층 사이에 끼는 층들이 **은닉층(hidden layer)**이고, 이 은닉층들의 차원만이 설계자가 미리 고르는 값 — **하이퍼파라미터(hyperparameter)** — 다. 예컨대 어른/아이 판별 신경망을 입력(2) → 은닉(5) → 은닉(3) → 출력(1)로 설계했다면, 각 층의 가중치 행렬은 앞서 본 읽기 규칙(행 = 출력 차원, 열 = 입력 차원)대로 5×2, 3×5, 1×3이다. 앞 행렬의 행 수(출력)가 다음 행렬의 열 수(입력)로 맞물리면서, 특징이 2 → 5로 늘었다가 5 → 3 → 1로 좁혀진다.

반드시 좁혀 가야만 하는 것도 아니다. 입력(784) → 128 → 32 → 128 → 출력(784)처럼 중간에서 차원을 확 줄여 핵심만 남겼다가(압축) 다시 원래 차원으로 되돌리는(복원) 구조가 **오토인코더(AutoEncoder)**인데, [오토인코더 개념 글]({% post_url 2020-07-13-AI-AE-01 %})에서 다룬 적이 있다. 복원이 목표인 문제라 최종 출력 차원이 입력 차원과 같아진 경우이고, 그 사이에서는 차원이 병목까지 줄었다가 도로 늘어난다. 뉴런이 특징을 새로 만들어 내는 장치라는 관점에서 보면, **층을 쌓는다는 것은 특징 개수를 늘리고 줄여 가며 표현을 바꿔 나가는 과정**인 셈이다.

> *참고*: 학습된 표현과 사람의 해석
>
> 앞서 "표현과 해석" 참고에서 층의 출력에 해석 대신 표현이라는 말을 쓰는 이유를 짚었는데, 여기서 한 발 더 나아가면 — 은닉층의 표현에는 애초에 사람의 해석을 기대하지 않는 편이 안전하다. "덩치가 큰가" 같은 이름은 설명을 위한 비유일 뿐, 실제 학습된 은닉층의 특징 하나하나가 **사람 말로 옮길 수 있는 개념과 대응한다는 보장은 없다**. 모델은 최종 오차를 줄이는 데 유리한 표현을 만들 뿐, **사람이 알아볼 수 있는 형태로 만들 의무가 없기 때문이다**. 은닉층에 뉴런을 5개 둔다는 것도 판단 기준 다섯 가지를 사람이 정해 넣는 일이 아니라, 표현을 담을 그릇의 크기를 정하는 일에 가깝다 — "그 5개가 각각 무슨 기준이냐"는 질문에는 답이 없다.
>
> 앞의 고양이 그림처럼 윤곽·눈 등 사람 개념과 맞아떨어지는 뉴런이 발견되기도 하지만, 그것은 학습이 끝난 모델을 들여다보고 알게 된 사후 관찰이지 설계자가 정해 넣은 역할이 아니다(한 뉴런에 여러 개념이 겹쳐 있는 경우도 많다). 그러니 **중간 층의 입출력에 사람의 해석을 일일이 붙여 이해하려 하면 오히려 헷갈린다**. 학습된 표현을 사람이 이해해 보려는 시도 자체가 하나의 연구 분야이니, 궁금하다면 interpretability(해석 가능성), 특히 mechanistic interpretability를 살펴보면 좋다. 응용·규제 맥락에서는 모델 내부를 여는 대신 판단 결과에 사후 설명을 붙이는 쪽에 무게를 둔 **Explainable AI(XAI)**라는 이웃 용어로도 만나게 된다.

## MLP와 딥러닝

지금까지 그린 구조를 한 문장으로 되짚어 보자. 
- ① 뉴런을 한 층에 여러 개 나란히 건 완전연결층 하나(= 선형변환, 행렬 곱)를 만들고, 
- ② 이 층을 여러 겹으로 쌓은 뒤(깊이), 
- ③ 층과 층 사이마다 활성화 함수를 끼운다. 
  
이 구조를 **MLP(Multi-Layer Perceptron, 다층 퍼셉트론)**라고 한다. ②·③을 하나의 식으로 쓴 것이 바로 앞에서 본 합성 함수 식($\hat{y} = W_3\,\sigma(\cdots) + b_3$)이고, 이게 곧 3층 MLP의 수식이다. 여기서 ③의 활성화 함수 $\sigma$를 빼 버리면 이 식은 $W_3W_2W_1x$이 되므로, 앞에서 본 것처럼 결국 하나의 직선일 뿐이다. MLP는 딥러닝의 가장 기본 골격이다. 이후 다룰 **Transformer 안에서도 어텐션과 번갈아 끼는 FFN(feed-forward network) 블록**이 정확히 이 "선형층 + 활성화" 구조다. 즉 여기서 본 골격 하나가 뒤에서 계속 재등장한다.
...
"행 = 데이터 개수, 열 = 특징"이라는 독법은 2D **데이터** 텐서에서의 관례일 뿐이다. 앞에서 본 가중치 행렬에서는 행 = 출력 뉴런, 열 = 입력이었다. 3D 이상의 텐서는 축(axis)마다 의미를 부여해 읽는다. 예를 들어 LLM의 입력 배치는 배치 × 시퀀스 길이 × 특징 차원의 3D 텐서다. 수학·물리학의 텐서(좌표 변환 규칙까지 포함하는 개념)와는 용법이 다르며, 딥러닝에서는 다차원 배열이라는 뜻으로만 쓰인다고 이해해도 지금은 충분하다.
