...
[이전 글]({% post_url 2026-01-18-Kubernetes-Kubeadm-01-0 %})에서 실습의 단계별 파일 맵을 정리했다. kubeadm은 `kubeadm init`과 `kubeadm join`이라는 두 명령어로 클러스터를 빠르게 구성할 수 있다고 했다.
...
`kubeadm init`이 완료되면 다음과 같은 결과물이 생성된다. 전체 실습에서의 파일 맵은 [실습 구성도]({% post_url 2026-01-18-Kubernetes-Kubeadm-01-0 %}#전체-디렉토리-트리)에서 확인할 수 있다.
