...
그런데 12월 한 달간의 사용량 데이터를 보고 솔직히 놀랐다. **320달러.** 

많은 금액인가, 적은 금액인가? 각자의 상황에 따라 다를 것이다. 이 글의 초점은, 금액의 크기가 아니다. 

중요한 것은, **무의식적으로 사용했다는 것**이다. 데이터를 분석해보니 68%의 최적화 여지가 보였다. 이 글은 그 발견과 개선을 향한 첫걸음이다.
...
Cursor 대시보드에서 12월 사용량 데이터를 CSV로 추출했다. 2025년 12월 5일부터 31일까지, 약 27일간의 데이터다. 주요 지표는 다음과 같다.
...
NotebookLM이 분석한 모델별 사용 패턴은 다음과 같다.

| 모델명 | 총 요청 건수 | 총 사용 토큰 | 총비용 ($) | 전체 비용 대비 비중 (%) |
| --- | --- | --- | --- | --- |
| `claude-4.5-opus-high-thinking` | 235 | 약 7천만 | $125 | **65.98%** |
| `claude-4.5-sonnet-thinking` | 377 | 약 7천만 | $63 | **33.51%** |
| `auto` | 20 | 약 3백만 | $0.97 | 0.51% |

<br>

`claude-4.5-opus-high-thinking` 모델이 전체 요청 건수의 약 36%를 차지했으나, 총비용에서는 **65.98%**라는 압도적인 비중을 차지했다. 단일 고비용 모델에 대한 의존도가 너무 높았다.
...
이 3일간 발생한 비용은 총 **$94.60**로, 전체 기간 비용의 **약 30%**를 차지한다. 단 3일간의 활동이 월 전체 비용의 1/3을 결정한 셈이다.

<br>

### 캐시 활용 패턴

흥미로운 점은 캐시 활용률이 매우 높았다는 것이다. NotebookLM 분석에 따르면:
...
Claude의 추정에 따르면, 적절한 컨텍스트 관리만 했어도 320달러가 102달러로 줄어들 수 있었을 것(약 68% 절감)이라고 한다.
...
즉, Included 방식이 On-Demand에 비해 압도적인 비용 효율성을 보인다. 특히 대규모 캐시 처리에서 큰 차이가 난다.
...
아래와 같은 경우, 새 대회를 시작하는 것을 고려해 보자:
- 작업 주제가 바뀔 때
...
320달러는 적지 않은 금액이다. 하지만 이 비용으로 대규모 리팩토링을 완료했고, 데이터 분석을 통해 68% 최적화 여지를 발견했다. **진짜 회고는 나중에 나온다.** 실험을 통해 실제로 비용이 줄어드는지, 품질은 유지되는지 검증할 것이다. 
