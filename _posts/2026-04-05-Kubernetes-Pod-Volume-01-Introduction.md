...
시나리오에 따라 적합한 볼륨 타입이 다르다.

| 시나리오 | 볼륨 타입 | 데이터 수명 |
| --- | --- | --- |
| 컨테이너 간 임시 파일 공유 | `emptyDir` | Pod 삭제 시 소멸 |
| 설정 파일 전달 | `configMap` | ConfigMap 오브젝트에 종속 |
| 민감 데이터 파일 전달 | `secret` | Secret 오브젝트에 종속 |
| Pod 메타데이터 파일 전달 | `downwardAPI` | Pod에 종속 |
| 이미지에서 파일 추출 | `image` | 이미지 레이어에 종속 |
| 노드 파일 접근 (시스템 Pod) | `hostPath` | 노드에 종속 |
| Pod와 독립적인 영구 데이터 | `persistentVolumeClaim` | PV에 종속 (Pod 삭제와 무관) |
...
Pod의 컨테이너는 보통 스토리지 **볼륨**을 동반한다. Kubernetes에서 볼륨은 Pod 내 컨테이너가 접근할 수 있는 디렉토리이며, 볼륨 타입에 따라 디렉토리의 생성 방식, 저장 매체, 초기 내용이 결정된다. 볼륨을 통해 Pod의 수명 동안(또는 그 이상) 데이터를 저장하거나, Pod 내 다른 컨테이너와 파일을 공유할 수 있다. ([Kubernetes 공식 문서: Volumes](https://kubernetes.io/docs/concepts/storage/volumes/))
...
컨테이너의 파일시스템은 **일회용(ephemeral)**이다. 이는 OCI 이미지의 구조에서 비롯된다. 컨테이너 이미지는 읽기 전용 레이어의 스택이고, 컨테이너가 시작되면 그 위에 얇은 쓰기 가능 레이어(writable layer)가 추가된다. 실행 중 파일을 수정하거나 생성하면 이 쓰기 가능 레이어에 기록되지만, 컨테이너가 삭제되면 이 레이어도 함께 사라진다.
...
### 볼륨과 자가 치유(self-healing)의 트레이드오프

볼륨에 데이터를 보존하면 컨테이너 재시작 후에도 상태가 유지되지만, 이것이 항상 바람직한 것은 아니다. 손상된 파일이 볼륨에 남아 있으면 새 컨테이너가 같은 손상된 파일을 읽고, 다시 크래시하고, 또 재시작하는 **무한 크래시 루프(CrashLoopBackOff)**에 빠질 수 있다.

판단 기준은 데이터의 성격이다.

- **상태(state) 데이터**: 데이터베이스 파일, 사용자 업로드 등 → 볼륨에 보존해야 한다
- **캐시/임시 데이터**: 로컬 캐시, 임시 파일 등 → 볼륨에 넣지 않는 것이 self-healing에 유리하다
...
Pod에 볼륨을 추가할 때는 **볼륨 타입**을 지정해야 한다. 크게 Pod의 수명 동안만 존재하는 **임시(ephemeral) 볼륨**과 Pod 삭제 후에도 유지되는 **영구(persistent) 볼륨**으로 나뉜다. ([Kubernetes 공식 문서: Ephemeral Volumes](https://kubernetes.io/docs/concepts/storage/ephemeral-volumes/))
...
Kubernetes 초기에는 `awsElasticBlockStore`, `gcePersistentDisk`, `nfs` 등 스토리지 기술별 볼륨 플러그인이 Kubernetes 코어에 내장(in-tree)되어 있었다. 새로운 스토리지를 지원하려면 Kubernetes 자체를 수정하고 릴리즈해야 했으므로 확장성에 한계가 있었다.

**CSI(Container Storage Interface)**는 이 문제를 해결하기 위해 도입된 표준 인터페이스다. 스토리지 벤더가 Kubernetes 코어와 독립적으로 드라이버를 개발/배포할 수 있게 해준다. 현재 기존 in-tree 플러그인은 점진적으로 deprecated되고, CSI 드라이버로 마이그레이션되고 있다.

주요 CSI 드라이버로는 AWS EBS CSI Driver, AWS EFS CSI Driver, Longhorn, Rook-Ceph 등이 있다. 이 시리즈에서 다루는 임시 볼륨(`emptyDir`, `configMap`, `secret` 등)은 CSI와 무관하지만, 영구 스토리지(`persistentVolumeClaim`)를 사용할 때는 CSI 드라이버가 핵심 역할을 한다.
