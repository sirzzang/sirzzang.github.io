---
title: "[EKS] EKS: Networking - 2주차 실습 환경 구성"
excerpt: "VPC CNI 실습을 위한 2주차 EKS 환경을 Terraform으로 구성하고, 1주차 대비 변경점을 확인해 보자."
categories:
  - Kubernetes
toc: true
header:
...
**IRSA**(IAM Roles for Service Accounts)는 **서비스 어카운트 단위로 IAM Role을 매핑**하는 메커니즘이다.
...
`configure_kubectl`은 Terraform 출력 변수명이지, 셸 명령이 아니다. 직접 치면 `command not found` 에러가 발생한다.
...
![EKS 콘솔 Overview - OIDC Provider URL 확인]({{site.url}}/assets/images/eks-w2-console-overview-oidc.png){: .align-center}
...
![EKS 콘솔 Compute - 노드 그룹 Kubernetes 라벨]({{site.url}}/assets/images/eks-w2-console-compute-nodegroup-label.png){: .align-center}

### Networking

서비스 IPv4 범위(10.100.0.0/16), 서브넷 목록, 클러스터 엔드포인트 접근 설정(public and private)을 확인할 수 있다.

![EKS 콘솔 Networking - 서비스 IP 범위, 서브넷, 엔드포인트 접근 설정]({{site.url}}/assets/images/eks-w2-console-networking.png){: .align-center}

### Add-ons

VPC CNI addon을 클릭하면 설정된 configuration values를 확인할 수 있다. Terraform에서 지정한 `WARM_ENI_TARGET` 설정이 반영되어 있다.

![EKS 콘솔 Add-ons - VPC CNI 설정값 확인]({{site.url}}/assets/images/eks-w2-console-addon-vpc-cni.png){: .align-center}
