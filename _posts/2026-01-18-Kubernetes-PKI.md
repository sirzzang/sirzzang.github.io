...
아래 내용은 **kubeadm**으로 클러스터를 프로비저닝하는 방식을 기준으로 설명한다. kubeadm은 공식 문서에서 권장하는 방식이며 PKI 구조를 이해하기에 가장 직관적이다. k3s, RKE, 수동 설치 등 다른 도구를 쓰면 인증서·키의 경로나 파일명이 달라질 수 있지만, CA 개수와 역할, 서버·클라이언트 인증서의 쓰임 등 기본 PKI 구조는 도구와 관계없이 일맥상통한다.
...
Kubernetes PKI의 CA 구조, 인증서 역할, 전체 인증서 목록을 살펴보자.
...
> **참고**: kubelet 관련 인증서
>
> kubelet은 API Server와 양방향으로 통신하므로, 서버 인증서와 클라이언트 인증서가 모두 필요하다.
>
> - **kubelet 서버 인증서**: API Server가 kubelet에 접속할 때(로그 조회, exec 등) kubelet이 제시하는 서버 인증서다. kubelet이 정당한 노드임을 API Server에 증명한다.
> - **kubelet 클라이언트 인증서**: kubelet이 API Server에 노드 상태를 보고할 때 사용하는 클라이언트 인증서다. CN은 `system:node:<nodeName>`, O는 `system:nodes`로 설정된다.
> - **apiserver-kubelet-client 인증서**: API Server가 kubelet에 접속할 때 API Server 쪽에서 제시하는 클라이언트 인증서다. apiserver용 인증서를 그대로 쓰는 방식(Shared)과 별도 클라이언트 인증서를 쓰는 방식(Separate)이 있으며, kubeadm은 Separate 방식으로 `apiserver-kubelet-client.crt` / `apiserver-kubelet-client.key`를 발급한다.
>
> kubelet의 서버·클라이언트 인증서는 다른 컴포넌트 인증서와 달리 `/etc/kubernetes/pki/`에 별도 파일로 존재하지 않는다. kubeadm은 TLS bootstrapping을 통해 kubelet 인증서를 자동 발급하며, 클라이언트 인증서는 `kubelet.conf`(kubeconfig)에 포함되고, 서버 인증서는 kubelet이 시작할 때 자체 생성하여 `/var/lib/kubelet/pki/` 디렉토리에 저장한다.
...
Kubernetes PKI에서 인증서는 역할에 따라 CA·서버·클라이언트 인증서로 구분된다. [공식 문서의 "모든 인증서" 표](https://kubernetes.io/ko/docs/setup/best-practices/certificates/#%EB%AA%A8%EB%93%A0-%EC%9D%B8%EC%A6%9D%EC%84%9C)에는 각 인증서의 **종류** 컬럼이 있어, Extended Key Usage(EKU)에 따라 server 전용, client 전용, 또는 둘 다 가능한(dual) 용도로 나뉜다.
...
- EKU에 `TLS Web Server Authentication`이 포함
- 접속을 **받는** 엔드포인트용
- CA에 의해 서명되며, 클라이언트는 CA 인증서로 서버 인증서를 검증
- 예: API Server(`apiserver.crt`)

### 클라이언트 인증서

사용자 또는 클러스터 컴포넌트의 신원을 증명한다.

- EKU에 `TLS Web Client Authentication`이 포함
- **한쪽 방향으로만** 접속하는 컴포넌트용
- CA에 의해 서명되며, 서버는 CA 인증서로 클라이언트 인증서를 검증
- 예: API Server가 etcd나 kubelet에 접속할 때 쓰는 클라이언트 인증서

### dual: 서버, 클라이언트 모두

통신에서 **서버이자 클라이언트** 역할을 동시에 하는 컴포넌트용이다. EKU에 `TLS Web Server Authentication`과 `TLS Web Client Authentication`이 **둘 다** 들어 있어, 같은 인증서가 상황에 따라 서버 인증서로 쓰이기도 하고 클라이언트 인증서로 쓰이기도 한다.

**예: etcd server.crt**

`/etc/kubernetes/pki/etcd/server.crt`는 dual이다. 동일한 인증서가 두 가지 방식으로 사용된다.

| 사용처 | 역할 | 설명 |
|--------|------|------|
| **etcdctl → etcd** | 클라이언트 인증서 | etcdctl이 `--cert=server.crt`, `--key=server.key`로 etcd에 접속할 때, etcd 서버가 "누구냐?" 하고 클라이언트 인증서를 검증(mTLS). 이때 `server.crt`는 **클라이언트의 신원 증명**이다. |
| **apiserver → etcd** | 서버 인증서 | etcd가 TLS 리스너에 `server.crt`, `server.key`를 바인딩해 두고, apiserver가 etcd에 접속하면 etcd가 이 인증서를 제시한다. apiserver는 CA(`ca.crt`)로 "이 etcd 서버가 진짜인가?" 검증한다. 이때 `server.crt`는 **서버의 신원 증명**이다. |
...
Kubernetes 클러스터에 필요한 인증서를 CA 인증서와 컴포넌트 인증서를 모두 포함해 정리한 목록이다. kubeadm으로 설치하면 대부분 `/etc/kubernetes/pki`에 저장된다.
...
아래 표는 공식 문서 [모든 인증서](https://kubernetes.io/ko/docs/setup/best-practices/certificates/#%EB%AA%A8%EB%93%A0-%EC%9D%B8%EC%A6%9D%EC%84%9C)에 나온 인증서를 **종류**(server 전용 / client 전용 / server·client dual)와 함께 정리한 것이다. 부모 CA, SAN(호스트) 등 상세는 공식 표를 참고하자.
...
CA 인증서 생성 → 컴포넌트별 CSR 생성 → CA가 서명하는 방식으로 구축하면 되며, [Kubernetes the Hard Way](https://github.com/kelseyhightower/kubernetes-the-hard-way) 문서가 이 방법을 상세히 설명한다. 실제 운영 환경에서는 권장되지 않지만, PKI 구조를 깊이 이해하는 데 도움이 된다.
...
아래는 **kubeadm**으로 설치한 클러스터 기준의 디렉토리 구조다. k3s·수동 설치 등 다른 방식에서는 경로와 파일명이 다를 수 있으나, 디렉토리 트리와 파일 역할은 이 글 앞에서 본 CA 구조·인증서 목록과 대응한다.
