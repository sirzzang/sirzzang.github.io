...
- GitHub Actions를 이용해 Elastic Beanstalk에 자동 배포하는 Workflow를 구성했다
- IAM 사용자를 생성하고, GitHub Secrets에 Access Key를 등록한 뒤, beanstalk-deploy action으로 배포한다
- 소스 코드를 `zip`으로 압축하여 S3에 업로드하고, EB 환경에 배포하는 구조다
...
현재 AWS에 EC2와 Load Balancer를 이용한 dev 환경, prod 환경을 구성했는데, 환경 세팅 및 관리를 위해 Elastic Beanstalk을 사용했다. EB 환경 운영 과정에서 겪었던 [배포 OOM 에러]({% post_url 2023-09-18-Dev-aws-elasticbeanstalk-deploy-oom-error %})나 [Nginx 413 에러]({% post_url 2025-01-20-Dev-AWS-Elastic-Beanstalk-Nginx-413-Error %}) 등의 트러블슈팅 경험도 있다. 각각의 환경에 앱을 배포할 때는 Elastic Beanstalk CLI를 이용하는데, 브랜치 전략은 논외로 하고, dev 환경에서는 dev 브랜치의 소스 코드를, prod 환경에서는 prod 브랜치의 소스 코드를 배포한다. 
...
on:
  push:
    branches:
    - dev # dev 브랜치에 push될 때 트리거
jobs:
  build:
    runs-on: ubuntu-latest
    steps:
    - name: Checkout source code # 소스 코드 체크아웃
      uses: actions/checkout@v2

    - name: Generate deployment package # 배포 패키지 생성
      run: zip -r deploy.zip . -i 'cmd/*' 'internal/*' Dockerfile go.*  
   
    - name: Get current time # 배포 버전 라벨에 사용할 현재 시각 생성
    ...
    - name: Deploy to EB # Elastic Beanstalk에 배포
...
> *참고*: Action 버전
>
> 이 글에서 사용한 `actions/checkout@v2`는 작성 시점의 버전이다. 최신 버전(v6)에서는 자격 증명 보안이 개선되었으므로, 새로 구성할 때는 최신 버전을 사용하는 것을 권장한다.
...
이 글에서는 IAM 사용자의 Access Key를 GitHub Secrets에 저장하는 방식을 사용했다. 동작에는 문제가 없지만, AWS에서는 장기 자격 증명(Access Key) 대신 **OIDC(OpenID Connect)**를 통한 임시 자격 증명 방식을 권장하고 있다.

OIDC 방식을 사용하면 다음과 같은 이점이 있다.

- Access Key를 발급하거나 주기적으로 교체할 필요가 없다
- Workflow 실행 시 임시 토큰을 발급받아 사용하므로, 자격 증명 유출 위험이 줄어든다
- IAM Role의 Trust Policy로 특정 repository, 브랜치에서만 접근하도록 제한할 수 있다

OIDC 방식으로 전환하려면, AWS에 GitHub용 OIDC Identity Provider를 등록하고, IAM Role을 생성한 뒤, [`aws-actions/configure-aws-credentials`](https://github.com/aws-actions/configure-aws-credentials) action을 사용하면 된다.
...
- [GitHub Actions에서 OIDC 설정하기](https://docs.github.com/en/actions/security-for-github-actions/security-hardening-your-deployments/configuring-openid-connect-in-amazon-web-services)
- [AWS 블로그: IAM roles to connect GitHub Actions](https://aws.amazon.com/blogs/security/use-iam-roles-to-connect-github-actions-to-actions-in-aws)
