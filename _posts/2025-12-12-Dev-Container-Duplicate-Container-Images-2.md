---
title:  "[Container] Docker와 containerd 이미지 관리 비교 - 2. 같은 이미지가 중복 저장된 이유"
...
노드에 장애가 생기지는 않았다. 다만, 습관처럼 노드에 접속해 루트 파티션 용량을 검사하다 70% 정도가 찬 것을 목격해, 어디 크기가 큰 파일이 있는지 확인하다가 발견한 문제다. 결과적으로는 **같은 이미지가 중복 저장되어 디스크가 낭비**되고 있던 셈이다.
...
- 보통 로컬에서 이미지를 빌드하게 되면, 동일한 Dockerfile을 이용해 빌드하더라도 [이미지 ID]({% post_url 2026-02-28-CS-Container-Image %}#이미지-id)가 달라진다.
- 레지스트리에서 이미지를 pull하게 되더라도, 컨테이너 런타임이 같은 이미지가 있는지 확인한다. 같은 이미지가 있을 경우, 다운로드하지 않는다.

대관절 어찌된 일인지 당황하지 않을 수 없다. 살펴 보니 기존 레거시 k3s 클러스터에서 컨테이너 런타임으로 Docker runtime을 이용하고 있다가, k3s에 내장되어 있는 containerd를 이용하도록 변경하면서 발생한 일이다. k3s 클러스터가 **프라이빗 레지스트리를 이용하도록 미러링 설정**되어 있기 때문에, 두 이미지는 모두 프라이빗 레지스트리에서 pull해온 것이다. 
...
## 서버 환경: Docker runtime과 k3s containerd의 공존

### 두 개의 containerd 인스턴스와 소켓
...
### 두 개의 데이터 디렉토리: k3s agent 디렉토리 구조

`/var/lib/rancher/k3s/agent/`는 k3s agent가 런타임 데이터를 저장하는 루트 디렉토리다. 각 하위 디렉토리의 역할은 다음과 같다:

- `containerd/`: k3s 내장 containerd의 데이터 디렉토리. 컨테이너 이미지 레이어, 스냅샷, 메타데이터 등이 저장된다.
- `cri-dockerd/`: Docker를 CRI 런타임으로 사용할 때 생성되는 디렉토리. Pod 샌드박스 체크포인트 정보가 저장된다.
- `etc/`: kubelet과 containerd의 설정 파일(인증서, kubeconfig 등)이 저장된다.
...
두 디렉토리가 모두 존재하는 이유는, k3s의 컨테이너 런타임을 전환할 때 **재설치가 아닌 재시작**만 했기 때문이다. k3s를 재설치하면 기존 디렉토리가 삭제되고 새로 생성되지만, 재시작만 했기 때문에 이전 런타임 설정에 따라 생성된 디렉토리가 그대로 남아있게 된 것이다.
...
결과적으로 현재는 k3s가 내장 containerd를 사용하고 있으므로, `containerd/` 디렉토리에 이미지가 저장되고 있다.
...
**각각의 컨테이너 런타임이 독립적으로 이미지를 관리**하기 때문이다. [컨테이너 파일 시스템]({% post_url 2026-03-01-CS-Container-Filesystem %}#구현체)에서 살펴보았듯이 Docker runtime과 containerd는 서로 다른 스토리지 드라이버 구현체를 사용하기 때문에 저장 구조도 다른데, 이미지 저장 경로가 다른 것이 직접적인 원인이다.
...
> 각 런타임의 상세한 디렉토리 구조는 [이전 글]({% post_url 2025-12-12-Dev-Container-Duplicate-Container-Images-1 %}#실제-디렉토리-구조-확인)에서 확인할 수 있다.
...
Docker runtime과 containerd가 어떻게 다른지, 이미지가 어디에 저장되는지, 각 CLI가 어떤 소켓으로 통신하는지를 알아야 문제 상황을 정확히 파악할 수 있다.
