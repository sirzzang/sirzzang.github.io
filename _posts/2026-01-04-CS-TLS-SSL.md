...
AEAD는 암호화와 인증을 동시에 수행하는 방식이다.

- **기존 방식 (TLS 1.2)**: 암호화(AES-CBC 등)와 인증(HMAC)을 별도로 수행
- **AEAD 방식 (TLS 1.3)**: 하나의 알고리즘으로 암호화와 인증을 동시에 처리
- **장점**: 성능 향상, 구현 단순화, 보안성 강화 (암호화와 인증의 순서로 인한 취약점 제거)
...
HTTPS는 **HTTP over TLS**이다. TLS 위에서 HTTP 통신을 수행한다.

## 연결 수립 과정

HTTPS 연결은 크게 두 단계로 나뉜다:
...
│  │  • 세션키(대칭키) 교환                               │  │
│  │  • 1회만 수행                                       │  │
│  │  • 소요 시간: 수십 ms (TLS 1.2), 수 ms (TLS 1.3)   │  │
│  └─────────────────────────────────────────────────────┘  │
│                          ↓                                 │
│  [2단계: 데이터 전송 - 대칭키 사용]                          │
│  ┌─────────────────────────────────────────────────────┐  │
│  │  • 세션키로 HTTP 데이터 암호화 (AES-GCM 등)          │  │
│  │  • 수천~수만 번 수행                                 │  │
│  │  • 오버헤드: 거의 없음 (대칭키는 매우 빠름)          │  │
│  └─────────────────────────────────────────────────────┘  │
│                                                            │
└────────────────────────────────────────────────────────────┘
```
...
**왜 두 단계로 나누는가?**
- 비대칭키: 느리지만 키 교환에 안전
- 대칭키: 빠르지만 키를 안전하게 공유하기 어려움
- 하이브리드 방식: 비대칭키로 대칭키를 안전하게 교환한 후, 대칭키로 빠르게 통신

<br>

## 세션키 생성 상세 (TLS 1.2)

클라이언트와 서버가 동일한 세션키를 생성하는 과정은 아래와 같다. TLS 원리를 차용한다.

```
[Client]                              [Server]
    │                                      │
    │  1. Generate clientRandom          │
    │     (32 bytes random)               │
    │────────────────────────────────────>│
    │     ClientHello (with clientRandom) │ Store clientRandom
    │                                      │
    │                                      │  2. Generate serverRandom
    │                                      │     (32 bytes random)
    │<────────────────────────────────────│
    │  ServerHello (serverRandom, cert)   │
    │  Store serverRandom                 │
    │                                      │
    │  3. Generate PMS (48 bytes random)   │
    │     Encrypt PMS with server         │
    │     public key                      │
    │────────────────────────────────────>│
    │     ClientKeyExchange               │ Decrypt PMS with server
    │                                      │ private key
    │                                      │
    ╔══════════════════════════════════════════════════════╗
    ║  At this point, both sides have same values:        ║
    ║  • clientRandom (32 bytes)                          ║
    ║  • serverRandom (32 bytes)                          ║
    ║  • PMS (Pre-Master Secret, 48 bytes)               ║
    ╚══════════════════════════════════════════════════════╝
    │                                      │
    │  4. Calculate Session Keys          │  4. Calculate Session Keys
    │  MasterSecret = PRF(PMS,            │  MasterSecret = PRF(PMS,
    │    "master secret",                 │    "master secret",
    │    clientRandom + serverRandom)    │    clientRandom + serverRandom)
    │                                      │
    │  SessionKeys = PRF(MasterSecret,    │  SessionKeys = PRF(MasterSecret,
    │    "key expansion",                  │    "key expansion",
    │    serverRandom + clientRandom)    │    serverRandom + clientRandom)
    │                                      │
    ╔══════════════════════════════════════════════════════╗
    ║  Same input → Same session keys guaranteed!         ║
    ╚══════════════════════════════════════════════════════╝
    │                                      │
```
...
- **PMS (Pre-Master Secret)**: 클라이언트가 생성하는 48바이트 랜덤 값
- **Master Secret**: PMS로부터 유도된 48바이트 비밀 값
- **PRF (Pseudo Random Function)**: 결정론적 함수. 같은 입력이면 항상 같은 출력을 생성

### 실제 생성되는 키

세션키는 사실 여러 개의 키로 구성된다:

1. **클라이언트 쓰기 MAC 키**: 클라이언트가 보내는 데이터의 HMAC 생성
2. **서버 쓰기 MAC 키**: 서버가 보내는 데이터의 HMAC 생성
3. **클라이언트 쓰기 암호화 키**: 클라이언트가 보내는 데이터 암호화
4. **서버 쓰기 암호화 키**: 서버가 보내는 데이터 암호화
5. **클라이언트 쓰기 IV**: 클라이언트 암호화용 초기화 벡터
6. **서버 쓰기 IV**: 서버 암호화용 초기화 벡터

<br>

## 왜 같은 세션키가 생성되는가?

양쪽이 같은 세션키를 생성할 수 있는 이유:

1. **PMS**: 클라이언트가 생성 → 서버 공개키로 암호화 → 서버가 개인키로 복호화 → **동일한 값 공유**
2. **clientRandom**: 평문으로 전송 → **동일한 값 공유**
3. **serverRandom**: 평문으로 전송 → **동일한 값 공유**
4. **PRF**: 결정론적 함수 → **같은 입력이면 항상 같은 출력**

```python
# 의사 코드
# 클라이언트 측
masterSecret = PRF(PMS, "master secret", clientRandom + serverRandom)
sessionKeys = PRF(masterSecret, "key expansion", serverRandom + clientRandom)
...
- **매 세션마다 다른 키 생성**: 키 재사용 공격 방지
- **양쪽이 모두 난수 기여**: 한쪽이 악의적이어도 안전
- **Replay attack 방지**: 과거 패킷을 재전송해도 세션키가 다르므로 무효
...
```
Client → Server:
1. Encrypt data with session key (AES-CBC 등)
2. Generate HMAC with MAC key
3. Send [Encrypted Data + HMAC]

Server:
1. Verify HMAC with MAC key
...
TLS 1.3에서는 AEAD(예: AES-GCM, ChaCha20-Poly1305)가 필수다.

```
Client → Server:
1. Encrypt and authenticate in one operation (AEAD)
2. Send [Ciphertext + Authentication Tag]

Server:
1. Decrypt and verify in one operation (AEAD)
2. If authentication fails, reject
```
- 암호화와 인증을 한 번에 수행 (더 빠름)
...
TLS 핸드셰이크는 상대적으로 비용이 크므로, 이전 연결을 재사용하는 메커니즘이 있다.

### Session ID

서버가 세션 ID를 발급하고, 클라이언트가 다음 연결 시 이를 제시하면 핸드셰이크를 생략할 수 있다.
...
[Abbreviated handshake - 키 재사용]
```

### Session Ticket (TLS 1.2)

서버가 세션 정보를 암호화하여 클라이언트에게 티켓으로 발급한다. 서버는 상태를 저장하지 않아도 된다 (Stateless).

### 0-RTT (TLS 1.3)

TLS 1.3에서는 이전 세션 정보를 사용하여 핸드셰이크 없이 바로 데이터를 전송할 수 있다 (0-RTT).

**주의**: 0-RTT는 Replay attack에 취약하므로 멱등성(idempotent) 요청에만 사용해야 한다.
...
**HTTPS**는 HTTP over TLS로, 포트 443을 사용하며 웹 보안 통신의 표준이다. **FTPS**는 FTP over TLS로 파일 전송을 암호화한다. **SMTPS**는 SMTP over TLS로 이메일 전송을 암호화한다 (포트 465 또는 587). **WSS**는 WebSocket over TLS로 실시간 양방향 통신을 암호화한다. **gRPC**는 HTTP/2 over TLS를 사용하며 mTLS를 지원한다. gRPC는 마이크로서비스 간 통신에 널리 사용되며, Protocol Buffers를 사용한 효율적인 직렬화와 스트리밍을 지원한다.
...
**HTTPS:**
- HTTP over TLS로, 웹 보안 통신의 표준이다
- 핸드셰이크(비대칭키)와 데이터 전송(대칭키)으로 나뉜다
- 하이브리드 암호화로 보안과 성능을 모두 달성한다
...
이것으로 **암호화 및 보안 프로토콜 시리즈**가 마무리된다.

요약:
- **암호화 기초**: 해시, 대칭키, 비대칭키
- **인증서와 PKI**: [디지털 인증서](/cs/CS-Security-Certificate/), [X.509](/cs/CS-X509-Certificate/), [PKI](/cs/CS-PKI/)
- **TLS/HTTPS**: 실제 보안 통신 프로토콜

이 개념들은 HTTPS 웹 통신, 쿠버네티스 인증, API 보안, 마이크로서비스 아키텍처 등 다양한 분야에서 활용된다.