...
excerpt: "Kubespray가 제공하고자 하는 것과 자동화 도구의 양면성을 살펴본다."
categories:
  - Kubernetes
toc: true
header:
...
[Kubernetes The Hard Way]({% post_url 2026-01-05-Kubernetes-Cluster-The-Hard-Way-00 %})에서는 클러스터의 모든 구성 요소를 손으로 설치했다. [kubeadm]({% post_url 2026-01-18-Kubernetes-Kubeadm-00 %})은 인증서 생성, etcd 구성, 컨트롤 플레인 배포 등 핵심 부트스트래핑 작업을 자동화해주었다. 이번 4주차에는 **Kubespray**로 전체 과정을 자동화해 본다.
...
Kubespray는 Kubernetes [SIG(Special Interest Group)](https://github.com/kubernetes/community/blob/master/sig-cluster-lifecycle/README.md) 중 **Cluster Lifecycle** 그룹에서 관리하는 공식 프로젝트다.

## 정의

Kubespray는 **Ansible 기반으로 쿠버네티스 클러스터를 자동으로 설치·업그레이드·관리하기 위한 오픈소스 배포 도구**다.
...
Kubespray는 업그레이드 기능(`upgrade-cluster.yml`)을 제공한다. 하지만 이것도 결국 Ansible Playbook이다. 무엇이 어떻게 바뀌는지 모르면:
...
Kubespray도 계속 발전한다. 새 버전이 나올 때마다:
