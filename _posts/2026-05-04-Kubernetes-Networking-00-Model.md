...
excerpt: "쿠버네티스 네트워킹의 4가지 문제 — 컨테이너 간, 파드 간, 파드-서비스, 외부-서비스 — 와 핵심 원칙인 'NAT 없이'가 어디까지 적용되는지를 정리한다."
categories:
  - Kubernetes
toc: true
header:
...
여기서 정의한 모델을 토대로, [파드 간 통신]({% post_url 2026-05-04-Kubernetes-Networking-01-Pod-to-Pod %})에서는 그 구현 방식을, [CNI 표준]({% post_url 2026-01-05-Kubernetes-CNI %})·[CNI 동작 흐름]({% post_url 2026-03-19-Kubernetes-CNI-Flow %})에서는 표준과 동작을, [Service와 kube-proxy]({% post_url 2026-05-04-Kubernetes-Networking-04-Service %})에서는 그 위에 얹히는 추상화 계층을 차례로 다룬다.
...
- [CNI 표준]({% post_url 2026-01-05-Kubernetes-CNI %}): 컨테이너 런타임과 네트워크 플러그인 사이의 인터페이스 표준
- [CNI 동작 흐름]({% post_url 2026-03-19-Kubernetes-CNI-Flow %}): Flannel(VXLAN) 클러스터에서 Pod 생성부터 노드 간 통신까지 패킷이 흐르는 과정
