...
- Docker : dockerd → containerd → runc → container
  ...
  - 따라서 Docker 사용 시, 아래와 같은 파일을 찾아볼 수 있음
...
> 
> 컨테이너 런타임에 대해 알아보다가 대관절 궁금해진 것. 어차피 contaienrd를 이용하면, 대관절 사람들은 왜 containerd를 안 쓰고 Docker를 사용할까? 결국 Docker는 containerd를 한 번 래핑해 놓은 것이 아닌가?
> 
> 이에 대해 찾아 보니, 역사적인 이유가 있다고 한다. 최초 Docker의 전성기 때, 즉, 컨테이너 솔루션으로 Docker가 최전성기를 달리고 있을 때, containerd는 Docker의 일부였다고 한다. Kubernetes도 컨테이너 오케스트레이션 솔루션이었으니, Docker에 의존할 수 밖에 없었고. 그런데, 2015년부터 Docker 내부 구조가 슬슬 분리되기 시작했다고 한다. dockerd와 containerd로, 그리고 OCI 표준화에 의해 runc로. 그러다 2020년대에 Kubernetes가 Docker 지원을 중단하게 되면서, 대대적인 변화가 나타났다고 한다.
> 
...
> 
> 어쨌든, 재미있는 이야기다. 나중에 시간 나면 역사 공부하듯 재미 삼아 찾아 봐도 좋을 듯하다.
...
> [공식 문서](https://developer.nvidia.com/container-runtime)에 의하면, GPU aware container runtime, compatible with the Open Containers Initiative(OCI) specification used by Docker, CRI-O, and other popular container techonologies로, OCI 런타임과 호환된다는 것이 명시되어 있다.

구조적으로는 **runc(저수준 컨테이너 런타임)에 GPU 기능을 추가**해 놓은 런타임이다. runc의 NVIDIA wrapper인 셈이다. 실제 containerd의 설정 파일 예시: runc 기반의 래퍼임을 확인할 수 있다.
```toml
[plugins."io.containerd.grpc.v1.cri".containerd.runtimes.nvidia]
runtime_type = "io.containerd.runc.v2"  # runc 기반

[plugins."io.containerd.grpc.v1.cri".containerd.runtimes.nvidia.options]
  BinaryName = "/usr/local/nvidia/toolkit/nvidia-container-runtime"  # NVIDIA 래퍼
```
...
- **GPU 디바이스 파일 마운트**: `/dev/nvidia0`, `/dev/nvidiactl`, `/dev/nvidia-uvm` 등 GPU 디바이스 파일을 컨테이너 네임스페이스에 마운트
...
즉, **libnvidia-container**가 실제 GPU 연결 로직을 구현하고, **nvidia-container-cli**가 이 라이브러리를 사용하며, **nvidia-container-runtime-hook**이 이 모든 과정을 OCI 표준에 맞게 진행된다.

<br>

# 동작 구조

컨테이너 환경에서의 컨테이너 런타임 동작 원리만 알면, 쿠버네티스 환경으로 확장하는 것은 쉽다.
...
- kubelet (CRI) → containerd → → VM → 컨테이너
  - kata-runtime의 경우, 경량 가상머신으로 격리되어 있다고 함

## Kubernetes, w/ GPU
- kubelet (CRI) → dockerd → containerd → nvidia-container-runtime → runc → 컨테이너
