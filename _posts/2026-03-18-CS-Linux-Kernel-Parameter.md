...
- **커널 파라미터 설정값**: `/etc/sysctl.d/`의 `.conf` 파일들은 **ASCII 코드값 기준 문자열(lexicographic) 정렬 순서**로 로드되며, 같은 파라미터가 여러 파일에 있으면 **나중에 읽힌 값이 이긴다**. 숫자(`0`-`9`) → 대문자(`A`-`Z`) → 소문자(`a`-`z`) 순서이므로, 문자로 시작하는 파일(예: `k8s.conf`)은 숫자 접두사 파일(`00-*` ~ `99-*`)보다 나중에 로드된다.
...
커널 파라미터의 영구 설정은 여러 디렉토리에 분산될 수 있다. `sysctl --system` 실행 시(또는 부팅 시) 아래 디렉토리들에서 `.conf` 파일을 수집한다. 나중에 스캔되는 디렉토리가 더 높은 우선순위를 가진다.

| 스캔 순서 | 경로 | 용도 |
| --- | --- | --- |
| 1 (가장 먼저) | `/usr/lib/sysctl.d/` | 패키지·배포판 기본값 (패키지 관리자가 관리, 직접 수정 비권장) |
| 2 | `/run/sysctl.d/` | 런타임 생성 설정 (임시, 재부팅 시 사라짐) |
| 3 (가장 나중) | `/etc/sysctl.d/` | **관리자 커스텀 설정** (가장 많이 사용, 우선순위 최상위) |
| 특수 | `/etc/sysctl.conf` | 전통적 설정 파일 (항상 마지막에 적용) |

바로 뒤에서 다루겠지만, 이 디렉토리 우선순위는 **동명 파일이 여러 디렉토리에 존재할 때** 어느 쪽을 채택할지 결정하는 데 사용된다. 관리자가 커스텀 설정을 넣을 때는 가장 우선순위가 높은 `/etc/sysctl.d/`에 파일을 추가하는 것이 권장된다.

## 로드 순서: 채택과 머지

설정 파일이 최종 적용되기까지는 **채택**과 **머지** 두 단계를 거친다.

### 1단계: 채택 — 동명 파일의 디렉토리 우선순위

여러 디렉토리에 **같은 이름의 파일**이 존재하면, 디렉토리 우선순위에 따라 하나만 채택되고 나머지는 무시된다. 우선순위는 `/etc/sysctl.d/` > `/run/sysctl.d/` > `/usr/lib/sysctl.d/` 순이다.

```
/usr/lib/sysctl.d/50-default.conf   ← 무시 (동명 파일이 /etc/에 있음)
/etc/sysctl.d/50-default.conf       ← 채택
```

파일명이 다르면 이 단계는 관여하지 않는다. 각 디렉토리의 파일이 모두 채택된다.

> 이 동명 파일 오버라이드는 패키지가 설치한 `/usr/lib/sysctl.d/` 파일을 직접 수정하지 않고 대체하기 위한 메커니즘이다. 패키지 파일을 직접 고치면 패키지 업데이트 시 덮어쓰여 버리기 때문에, `/etc/sysctl.d/`에 같은 이름의 파일을 만들어 통째로 대체하는 것이다. 이 용도가 아니라면 같은 이름의 파일을 여러 디렉토리에 분산해 둘 이유는 없다. 우선순위가 낮은 쪽이 조용히 무시되므로, 의도치 않게 설정이 누락되는 원인이 될 수 있다.

### 2단계: 머지 — 파일명 기준 정렬·적용

채택된 파일들을 **파일명의 문자열(lexicographic) 정렬 순서대로** 적용한다. 어떤 디렉토리에서 왔는지는 무관하다. 같은 파라미터가 여러 파일에 있으면 나중에 적용된 파일의 값이 덮어쓴다. `/etc/sysctl.conf`만 예외로 항상 가장 마지막에 적용된다.

예를 들어 채택된 파일이 아래와 같다면:

```
/etc/sysctl.d/00-defaults.conf               # 파일명: 00-defaults.conf
/usr/lib/sysctl.d/10-default-yama-scope.conf # 파일명: 10-default-yama-scope.conf
/etc/sysctl.d/99-amazon.conf                 # 파일명: 99-amazon.conf
```

로드 순서는 `00-defaults.conf` → `10-default-yama-scope.conf` → `99-amazon.conf`다. `/etc/sysctl.d/`가 `/usr/lib/sysctl.d/`보다 우선순위가 높은 디렉토리이지만, 머지 단계에서는 파일명 `00-*`이 `10-*`보다 앞이므로 `/etc/`의 파일이 먼저 적용된다.

## 파일명 규칙

파일명에 대해 systemd가 강제하는 형식 제한은 없다. `100-custom.conf`, `999-override.conf` 같은 세 자리 이상의 숫자도 유효하고, 숫자 없이 `custom.conf`처럼 문자로 시작하는 파일명도 정상적으로 로드된다.

문자열 정렬 순서는 각 문자의 **ASCII 코드값**에 의해 결정된다.
...
숫자가 알파벳보다 앞에 오는 것도 ASCII 코드값이 더 작기 때문이다. 따라서 숫자로 시작하는 파일이 먼저 로드되고, 대문자, 소문자 순서로 뒤따른다. 예를 들어 `00-default.conf`, `99-k8s.conf`, `k8s.conf` 세 파일이 있으면:

```
# 로드 순서
00-default.conf    # '0' = ASCII 48 → 가장 먼저
99-k8s.conf        # '9' = ASCII 57 → 두 번째
k8s.conf           # 'k' = ASCII 107 → 가장 나중
```

`k8s.conf`가 가장 나중에 읽히므로, 같은 파라미터가 겹치면 `k8s.conf`의 값이 최종 승리한다.

## 번호 대역 관례

실무에서는 관례적으로 **00-99 두 자리 숫자**를 사용한다. 대부분의 배포판과 패키지가 두 자리 컨벤션을 따르고, 두 자리 범위에서는 문자열 순서와 숫자 순서가 일치하므로 직관적이다. 번호 대역 별 용도도 관례적으로 정해져 있다.
...
이 관례에서 앞서 설명한 **문자열 기준 정렬**은 의외로 중요하다. 두 자리 숫자(00-99) 범위 안에서는 문자열 순서와 숫자 순서가 일치하므로 문제가 없지만, 세 자리 이상의 숫자를 섞으면 의도와 정반대로 동작할 수 있다.
...
위 예시에서 `100-custom.conf`는 "99보다 뒤에 적용되어야 할 가장 중요한 설정"이라는 의도로 만들었을 수 있지만, 문자열 `"1"` < `"2"`이므로 `20-defaults.conf`보다도 **앞에** 로드되어 오히려 초반에 덮어쓰여 버린다. 숫자상 100이 99보다 크다는 직관이 문자열 정렬에서는 통하지 않는 것이다.

실무에서 세 자리 이상의 번호를 써야 할 상황은 거의 없다. 만약 그런 필요가 느껴진다면 설정 파일이 너무 많아져 관리 체계 자체를 재고해야 할 신호이고, 쓴다 하더라도 대부분의 배포판·패키지가 따르는 두 자리 컨벤션과 충돌하여 예측하기 어려운 로드 순서가 만들어질 수 있다.
...
`sysctl --system`은 설정을 적용하면서 동시에 **어떤 파일을 어떤 순서로 로드했는지** 출력한다. 즉 적용과 확인이 한 번에 이루어지는 명령어다. 이 출력을 통해 위에서 설명한 정렬 규칙이 실제로 어떻게 적용되는지 확인할 수 있다. 아래는 EKS 워커 노드(Amazon Linux 2023)에서의 실제 출력 예시다.
...
다만, 설정 파일을 만드는 것만으로는 커널에 즉시 반영되지 않는다. `/etc/sysctl.d/`의 파일은 디스크에 저장된 텍스트 파일일 뿐이고, 커널이 이 파일을 실시간으로 감시하지 않기 때문이다. systemd 유닛 파일을 수정해도 `systemctl daemon-reload`를 해야 systemd가 인식하듯, sysctl 설정 파일도 명시적으로 로드해야 커널에 반영된다.

```bash
# 모든 디렉토리의 설정 파일을 로드 순서대로 다시 적용
sysctl --system
```

| 명령어 | 동작 |
| --- | --- |
| `sysctl --system` | 모든 디렉토리의 `.conf` 파일을 로드 순서대로 읽어 커널에 적용 |
| `sysctl -p <파일>` | 특정 파일 하나만 로드하여 커널에 적용 |

정리하면, "지금 당장 + 영구적으로" 적용하려면 **파일 작성과 로드를 세트로** 해야 한다. 파일 작성만 하면 다음 재부팅까지 적용되지 않고, `sysctl -w`로 런타임 변경만 하면 재부팅 시 사라진다.
...
컨테이너는 VM과 달리 **호스트 커널을 공유**한다. 내부에서 별도 커널을 실행하는 것이 아니라 호스트 커널에서 실행되는 프로세스일 뿐이므로, 호스트의 커널 파라미터가 모든 컨테이너의 동작에 직접 영향을 미친다.
...
## 설정 파일명 관례: k8s.conf vs 99-k8s.conf

한편, [Kubernetes 공식 문서의 Container Runtimes 페이지](https://kubernetes.io/docs/setup/production-environment/container-runtimes/#prerequisite-ipv4-forwarding-optional)에서는 IP 포워딩 설정을 `99-k8s.conf`가 아니라 `k8s.conf`에 넣도록 안내한다.
...
이 선택은 앞서 설정 파일 체계에서 설명한 ASCII 코드 기반 정렬 규칙을 활용한 것이다. 문자 `k`(ASCII 107)는 숫자 `9`(ASCII 57)보다 뒤에 오므로, `k8s.conf`는 숫자 접두사가 붙은 모든 파일(`00-*` ~ `99-*`) 뒤에 자연스럽게 로드된다. 번호 규칙을 고민할 필요 없이, **숫자 접두사 파일 전체보다 높은 우선순위**를 확보하는 셈이다.

정리하면:
- `99-k8s.conf` → 번호 컨벤션 중 가장 마지막 대역(99)을 명시적으로 잡는 방식
- `k8s.conf` → 번호를 쓰지 않아도 숫자 뒤에 오므로, 사실상 `99-*`보다도 뒤

두 방식 모두 "나중에 읽혀서 이긴다"는 목적은 같지만, `k8s.conf`가 더 간결하면서도 더 높은 우선순위를 가진다. 다만 `/etc/sysctl.conf`는 파일명 순서와 무관하게 항상 가장 마지막에 적용되는 특수 케이스라는 점은 유의해야 한다.

## Pod 수준 sysctl

호스트 커널을 공유한다고 해서 Pod에서 커널 파라미터를 전혀 조정할 수 없는 것은 아니다. Pod의 `securityContext.sysctls`를 통해 일부 파라미터를 Pod 단위로 설정할 수 있다. 다만 모든 파라미터가 가능한 것은 아니며, **어떤 파라미터를 설정할 수 있는지**는 두 가지 관점에서 결정된다.
...
### Safe vs Unsafe (namespaced 파라미터의 세분류)
