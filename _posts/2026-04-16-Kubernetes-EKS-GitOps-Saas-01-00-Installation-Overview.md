...
상세 내용은 [CloudFormation 코드 분석]({% post_url 2026-04-16-Kubernetes-EKS-GitOps-Saas-01-01-01-Installation-CloudFormation %})에서 다룬다.
...
상세 내용은 [install.sh 코드 분석]({% post_url 2026-04-16-Kubernetes-EKS-GitOps-Saas-01-01-02-Installation-Install-Script %})에서 다룬다.
...
# 참고: Gitea, Flux, OpenTofu, Tofu Controller

## Gitea

**[Gitea](https://about.gitea.com/)**는 Go로 작성된 셀프호스팅 경량 Git 서버다. GitHub/GitLab과 같은 기능(저장소 관리, 이슈 트래킹, CI/CD 등)을 제공하지만, 단일 바이너리로 실행할 수 있어 리소스 소비가 적다.

이 워크숍에서 Gitea는 **GitOps의 Single Source of Truth** 역할을 한다. 참가자가 Gitea repo에 코드를 push하면 Flux가 변경을 감지하여 EKS 클러스터에 반영하는 구조다. GitHub 대신 Gitea를 사용하는 이유는 **워크숍 환경을 외부 서비스에 의존하지 않고 완전히 자체 완결적으로 구성하기 위함**이다. Gitea 서버 자체도 Terraform이 별도 EC2에 자동 배포하므로, 참가자가 별도로 GitHub 계정이나 토큰을 준비할 필요가 없다.
...
1. [CloudFormation 코드 분석]({% post_url 2026-04-16-Kubernetes-EKS-GitOps-Saas-01-01-01-Installation-CloudFormation %})
2. [install.sh 코드 분석]({% post_url 2026-04-16-Kubernetes-EKS-GitOps-Saas-01-01-02-Installation-Install-Script %})
3. [설치]({% post_url 2026-04-16-Kubernetes-EKS-GitOps-Saas-01-01-03-Installation %})
4. [설치 결과 확인]({% post_url 2026-04-16-Kubernetes-EKS-GitOps-Saas-01-01-04-Installation-Result %})
