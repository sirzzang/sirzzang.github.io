...
<center><sup>파드 생성 시 IP 할당 과정 — EC2 API 호출 없이 로컬에서 완결된다. 하단의 오른쪽 ENI(eth1)에서 화살표 하나가 New Pod로 연결되어 이번에 새로 할당되는 IP를 나타낸다.</sup></center>
...
<center><sup>IP 풀이 고갈되면 ipamd가 백그라운드에서 EC2 API를 호출해 새 ENI를 확보한다. 하단에서 eth0, eth1의 화살표는 이미 할당된 IP이고, 새로 추가된 eth2와 그 화살표가 이번에 EC2 API를 통해 확보한 ENI와 IP다. 오른쪽 점선 박스(New Pods)가 이 새 IP를 받아 생성될 파드를 나타낸다.</sup></center>
...
예를 들어 m5.large(ENI 3개, ENI당 슬롯 10개)는 `3 × (10 - 1) + 2 = 29`개(보조 IP 27개 + hostNetwork 2개), t3.small(ENI 3개, ENI당 슬롯 4개)은 `3 × (4 - 1) + 2 = 11`개(보조 IP 9개 + hostNetwork 2개)다. 이 공식은 **Secondary IP 모드 기준**이며, Prefix Delegation에서는 슬롯 하나에 /28 prefix(16개 IP)가 할당되므로 16배가 된다(`ENI 수 × (슬롯 수 - 1) × 16`).
...
위에서 설명한 슬롯 하나에 **개별 /32 IP 주소 1개**가 들어간다. 파드 1개 = IP 1개이므로, [위의 공식](#슬롯-ip-관리의-기본-단위)대로 m5.large는 보조 IP 27개를 가지며 max-pods는 29개다(hostNetwork 인프라 파드 2개 포함). 기본 모드이며, IP를 아껴 쓰는 소규모 클러스터에 적합하다.
...
두 축의 조합을 한눈에 정리하면 다음과 같다(m5.large 기준, ENI 3개 × 보조 IP 슬롯 9개):
