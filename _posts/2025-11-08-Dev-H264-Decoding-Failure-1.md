---
title:  "[Dev] 영상 업로드 실패 사태 분석 - 1. 홀수 해상도 문제와 트러블슈팅"
...
> - 다만, 더 엄밀한 의미에서 트랜스코딩(transcoding)은 코덱 변경을 의미한다. 광의의 개념에서 함께 지칭하는 해상도 변환은 트랜스사이징(transsizing), 비트레이트 변경의 경우 트랜스레이팅(transrating), 컨테이너만 변경하는 경우 트랜스먹싱(transmuxing)이라고 해야 한다.
...
*다음 포스트: [[Dev] 영상 업로드 실패 사태 분석 - 2. YUV 색공간과 영상 압축]({% post_url 2025-11-08-Dev-H264-Decoding-Failure-2 %})*