...
mTLS Handshake 과정 등 자세한 내용은 [TLS/SSL 프로토콜 글의 mTLS](/cs/CS-Security-TLS-SSL/#mtls)를 참고하자.
...
- 인증서 구조와 검증 과정에 대한 자세한 내용은 [X.509 인증서 글의 인증서 구조](/cs/CS-Security-X509-Certificate/#인증서-구조)를 참고하자.
- DN 체계(CN, O 등)에 대한 자세한 내용은 [X.509 인증서 글의 Distinguished Name](/cs/CS-Security-X509-Certificate/#distinguished-name-dn과-x500-표준)을 참고하자.
...
쿠버네티스 인증서에서 특히 중요한 필드가 있다. 이후 실습에서 `ca.conf` 파일을 작성할 때 이 필드들을 직접 만나보게 될 것이다.
...
- CA의 개인키(`ca.key`)는 컨트롤 플레인 노드의 `/etc/kubernetes/pki/` 디렉토리에 보관
