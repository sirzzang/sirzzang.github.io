...
GPU 같은 하드웨어 장치를 컨테이너에서 사용하려면, 호스트의 장치 파일을 컨테이너에 주입해야 한다. [Kubernetes 환경에서 NVIDIA GPU를 설정]({% post_url 2024-07-19-Dev-Kubernetes-GPU-Setting %})하거나, [NVIDIA Container Runtime의 동작 원리]({% post_url 2024-07-21-Dev-Nvidia-Container-Runtime %})를 파악하다 보면 `nvidia-container-runtime-hook`, `CDI` 같은 개념을 마주하게 된다.

이 글에서는 컨테이너 환경에서의 장치 주입을 위한 두 가지 방식인 **OCI Runtime Hook**과 **CDI(Container Device Interface)**를 살펴본다.

> 배경 지식:
> - [Everything is a File 철학]({% post_url 2026-01-31-CS-Everything-is-a-File %}): 장치를 `/dev/` 파일로 추상화하는 Linux의 설계 철학
> - [디바이스 드라이버 3계층 구조]({% post_url 2026-02-01-CS-Linux-Device-Driver %}): 커널 모듈(`.ko`) → 장치 파일(`/dev/*`) → 유저 라이브러리(`.so`)
>
> 컨테이너 장치 주입이란, 이 3계층 구성 요소를 호스트에서 컨테이너 네임스페이스 안으로 가져오는 것이다.
...
- **2017.07.**: OCI Runtime Spec 1.0.0 발표 - `prestart`/`poststart`/`poststop` Hook 포함
- **2019.12.**: `createRuntime`/`createContainer`/`startContainer` Hook 추가 (PR #1008)
...
Hook은 OCI 런타임이 읽는 `config.json`에 정의한다. `config.json`은 containerd나 CRI-O 같은 상위 런타임(CRI 구현체로, 컨테이너 생명주기를 관리)이 컨테이너 이미지와 실행 옵션을 기반으로 생성하며, runc 같은 OCI 런타임이 이를 읽어 컨테이너를 생성한다.
...
nvidia-container-runtime-hook prestart
  ↓
  ├─ mknod /dev/nvidia0          # 장치 파일 생성 (3계층: 장치 파일)
  ├─ mknod /dev/nvidiactl
  ├─ mount libcuda.so.1          # 유저 라이브러리 마운트 (3계층: 유저 라이브러리)
  ├─ mount libcudnn.so.8
  └─ setup cgroup devices        # 커널 모듈 접근 허용 (3계층: 커널 모듈)
  ↓ exit(0)
...
- **2017**: Kubernetes community에서 CDI 초기 제안 (PR #545)
- **2020**: CNCF TAG Runtime COD Working Group에서 CDI 레포 생성, 스펙 개발 시작
- **2021-2022**: CDI 스펙 안정화 (v0.3.0 → v0.5.0)
- **2023**: containerd 1.7+, CRI-O 1.24+ 공식 지원
- **2024**: CDI v1.0.0 릴리즈
...
> 참고: **Linux 철학과의 유사성**
>
> CDI의 설계는 어떻게 보면 Linux의 ["Everything is a file"]({% post_url 2026-01-31-CS-Everything-is-a-File %}#everything-is-a-file) 철학과 유사하다. 다양한 하드웨어를 일관된 추상화 계층으로 접근한다는 점에서 동일한 설계 철학이다.
...
- **2017-2022**: Kubernetes 대중화, CDI 스펙 제안 및 개발
- **2023-현재**: CDI가 새로운 표준으로 자리잡음 (v1.0.0 릴리즈)
