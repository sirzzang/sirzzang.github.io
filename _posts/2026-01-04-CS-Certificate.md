...
> 인증서의 대표적인 표준 형식인 X.509에 대해서는 [X.509 인증서](/cs/CS-X509-Certificate/)를, PKI 전반에 대해서는 [PKI](/cs/CS-PKI/)를 참고하자.
...
루트 인증서는 **CA가 스스로 발급**하고 **자신의 개인키로 서명**한 특별한 인증서이다.
...
일반 인증서와 Self-Signed 인증서의 차이를 살펴 보자.
```
일반 인증서: 개인키 생성 → CSR 생성 → CA에 서명 요청 → 인증서 발급
Self-Signed: 개인키 생성 → 자기 자신이 서명 → 인증서 생성
```
Self-Signed 인증서는 외부 CA 없이 직접 생성할 수 있다. 하지만 아무도 이 인증서를 "믿어도 된다"고 보증해주지 않기 때문에, 브라우저는 "연결이 안전하지 않음" 경고를 표시한다. 브라우저가 기본적으로 신뢰하는 CA 목록(Trust Store)에 포함되어 있지 않기 때문이다.

<br>

## 루트 인증서의 신뢰

루트 인증서도 Self-Signed다. 자기 자신이 서명했기 때문에, 기술적으로 "누가 이걸 보증하는가?"에 대한 답이 없다. 그래서 **Trust Anchor(신뢰의 닻)**라고 부른다. 신뢰 체인을 거슬러 올라가면 결국 이 지점에서 멈추고, "이건 그냥 믿기로 했다"가 된다.
