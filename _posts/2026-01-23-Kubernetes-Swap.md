...
`/etc/fstab` 파일에서 swap 라인을 주석 처리한다.

```bash
# /swap.img      none    swap    sw    0   0
```

또는 systemd swap 유닛을 비활성화해도 된다.
...
검증 항목별 설명과 에러 시 대응(예: `--ignore-preflight-errors=Swap`)은 [kubeadm init - preflight]({% post_url 2026-01-18-Kubernetes-Kubeadm-01-1 %}#1-preflight)을 참고하자.

<br>

# 스왑 비활성화가 권장되는 이유

쿠버네티스는 **노드 자원을 직접 통제하고 관리**할 수 있어야 한다. 스왑을 켜두면 메모리 부족 시 커널이 스왑으로 넘기면서 **그 통제권이 OS 쪽으로 넘어간다**. 클러스터의 안정성과 예측 가능한 스케줄링을 위해 스왑은 반드시 비활성화하는 것이 권장된다.
...
하지만 쿠버네티스는 **분산 시스템**이다. 단일 노드의 생존보다 **클러스터 전체의 안정성**이 중요하다. 기본 관리 철학은 **메모리가 부족하면 해당 Pod를 즉시 종료(OOMKilled)**하고 다른 노드에서 재시작하는 것이다.

- 일반 서버: 메모리 부족 → 스왑 사용 → 느리지만 작업 계속
- Kubernetes: 메모리 부족 → Pod 종료 (OOMKilled) → 다른 노드에서 재시작

스왑이 켜져 있으면 Pod가 죽지 않고 디스크를 쓰며 느린 상태로 버티기 때문에, **쿠버네티스 입장에서는 정상 Pod인지 비정상 Pod인지 구분하기 어렵다**. 이 철학이 깨지면서 아래와 같은 문제가 발생한다.
...
특수한 상황에서 스왑을 허용해야 한다면 아래와 같은 방법을 고려해 보자.
...
스왑을 허용할 경우 예측 가능한 리소스 동작이 깨질 수 있으므로, 노드·Pod 수준의 메모리·스왑 지표를 지속적으로 확인하고 성능 저하 시 즉시 대응할 수 있도록 모니터링을 강화해야 한다.
