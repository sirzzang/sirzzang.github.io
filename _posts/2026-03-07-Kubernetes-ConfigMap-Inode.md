...
> ConfigMap의 기본 개념, 생성 방법, 환경 변수 주입 등 전반적인 내용은 [어플리케이션 설정 - 2. ConfigMap]({% post_url 2026-04-05-Kubernetes-Application-Config-02-ConfigMap %}) 글을 참고한다. 이 글에서는 ConfigMap **볼륨 마운트의 inode 수준 동작**에 집중한다.
...
ConfigMap을 파드에서 사용하는 방법 중 하나는 볼륨으로 마운트하는 것이다. ConfigMap의 각 키가 **파일 이름**이 되고, 값이 **파일 내용**이 되어 컨테이너 파일 시스템에 나타난다. `nginx.conf`나 `application.properties`처럼 설정 파일 자체를 컨테이너 내부에 배치해야 할 때 쓴다. 환경 변수 주입과의 차이 등 기본 개념은 [ConfigMap 글]({% post_url 2026-04-05-Kubernetes-Application-Config-02-ConfigMap %}#볼륨-마운트-방식-미리보기)에서 다뤘다.
