...
누구나 Self-Signed 인증서를 만들 수 있지만, Trust Store에 포함되지 않으면 검증하는 측에서 신뢰하지 않는다. 기술적으로 "누가 이걸 보증하는가?"에 대한 답이 없어서 **Trust Anchor(신뢰의 닻)**라고 부른다. 신뢰 체인을 거슬러 올라가면 결국 이 지점에서 멈추고, "이건 그냥 믿기로 했다"가 된다.
