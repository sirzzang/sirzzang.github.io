---
title:  "[Kubernetes] Pod CPU Limit과 FFmpeg Thread 최적 조정 - 4.0. 문제 원인 파악: 실험"
...
| | A (로컬 Docker) | B (VM Docker) | C (K8s Pod) |
|---|---|---|---|
| real | 2.655s | 43.679s | **2m 30.302s** |
| user | 0.007s | 0.005s | 0.008s |
| sys | 0.027s | 0.019s | 0.031s |

### `curl -w` 결과

| | A (로컬 Docker) | B (VM Docker) | C (K8s Pod) |
|---|---|---|---|
| DNS | 0.000011s | 0.000012s | 0.000047s |
| Connect | 0.000388s | 0.000833s | 0.000651s |
| TTFB | 0.199s | 0.235s | 0.254s |
| Total | 2.639s | 37.983s | **151.762s** |

### 서버 처리 시간 (Total - TTFB)

| | A (로컬 Docker) | B (VM Docker) | C (K8s Pod) |
|---|---|---|---|
| 서버 처리 시간 | **2.44s** | **37.75s** | **151.51s** |
| A 대비 배율 | 1x | 15.5x | **62.1x** |
