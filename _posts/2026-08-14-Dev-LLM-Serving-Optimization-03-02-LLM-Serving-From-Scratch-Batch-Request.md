...
반면 vLLM의 **continuous batching**은 스케줄링 단위를 배치에서 **디코딩 스텝**으로 내린 방식이다. 매 스텝 끝난 시퀀스를 내보내고 대기 중인 요청을 빈자리에 채우므로, 배치 구성이 스텝마다 바뀔 수 있다.
...
앞의 [로그 관찰](#로그로-보는-41-분할)에서 "원칙적으로는 다른 사용자의 요청도 하나의 배치로 묶일 수 있는 구조"라고 했는데, 그 가능성이 실현되려면 요청 경계를 넘어 디코딩 스텝 단위로 배치를 재구성하는 스케줄링이 필요하다 — continuous batching이 하는 일이고, 이 시스템에서는 스트리밍 경로가 그 방향의 첫걸음이다.
