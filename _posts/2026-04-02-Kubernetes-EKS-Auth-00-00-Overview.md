...
# 근본 문제: 서로 모르는 두 인증/인가 체계의 만남

EKS가 풀어야 하는 본질적인 문제는, 독립적으로 설계된 두 인증/인가 체계가 하나의 플랫폼 위에서 돌아간다는 것이다.

## 인증의 불일치: 서로의 신분증을 모른다

|  | K8S 세계 | AWS 세계 |
| --- | --- | --- |
| **인증 주체 (신분증)** | User(X.509 CN/O 등) · ServiceAccount(JWT) | IAM User/Role + SigV4 |
...
- **사용자 관리는 이미 해결된 문제**: 기업 환경에는 LDAP, Active Directory, Okta 등 성숙한 IdP(Identity Provider)가 이미 있다. K8s가 자체 User 오브젝트를 만들면 기존 IdP와 동기화 문제(이중 관리), 비밀번호 정책/MFA/계정 잠금 등 IAM 기능의 자체 구현이 필요해져 "또 하나의 불완전한 사용자 관리 시스템(IdM)"이 된다
...
- **etcd 부담 최소화**: User 오브젝트를 etcd에 저장하면 사용자 수만큼 오브젝트가 늘어나고, 인증 시마다 etcd 조회가 필요해진다. 현재 구조에서는 Human User 인증(X.509, OIDC, webhook)이 etcd 조회 없이 stateless하게 처리되어 API 서버 성능에 유리하다

> 다만 K8s의 모든 인증이 stateless인 것은 아니다. ServiceAccount의 bound SA token(projected token)은 TokenReview API를 통해 토큰의 유효성(만료, 삭제 여부)을 확인하므로 etcd 조회가 발생한다. 그러나 이는 K8s 내부 오브젝트인 Pod identity의 검증이고, Human User 인증은 etcd 조회 없이 처리되므로 "User 오브젝트가 없어서 etcd 부담이 줄어든다"는 논지는 유효하다.

### 이 설계가 EKS에 주는 의미

K8s에 User가 없는 것 자체는 문제가 아니다. 오히려 EKS가 기존 AWS IAM 체계를 **중복 구축 없이** 그대로 활용할 수 있게 해주는 설계적 장점이다. K8s의 authentication은 결국 `UserInfo{Username, Groups, UID}`를 반환하는 인터페이스이고, 그 뒤의 RBAC은 이 문자열만 본다. EKS는 webhook authenticator를 통해 IAM identity를 이 문자열로 변환할 뿐이다.

## 인가의 불일치: 권한 모델의 미스매치

인증 주체가 서로 다른 것에 더해, 인가(Authorization) 모델의 도메인 자체가 다르다. IAM Policy로 "pods를 get할 수 있다"를 표현할 수 없고, K8s 인가 체계로 "특정 IP에서만 접근"을 표현할 수 없다. 이 미스매치는 RBAC뿐 아니라 ABAC, Webhook 등 어떤 K8s authorization mode를 쓰더라도 동일하다.
...
| 단계 | 질문 | 입력 | 출력 | 인증/인가 |
| --- | --- | --- | --- | --- |
| **1. 신뢰 수립**(Trust Establishment) | 상대 세계의 credential을 믿을 수 있는가? | 상대 세계의 credential (토큰, pre-signed URL 등) | "이 credential은 유효하다" (검증된 source identity) | **인증** |
| **2. 신분 매핑**(Identity Mapping) | 검증된 상대를 우리 세계의 누구로 대응시킬 것인가? | 검증된 source identity | target 세계의 identity (인가 정책이 평가할 주체) | **인증 → 인가 브릿지** |
| **3. 최소 권한**(Least Privilege) | 매핑된 주체에 적절한 권한만 부여되었는가? | 매핑된 target identity | 허용 / 거부 | **인가** |
...
진짜 복잡성은 "두 개의 독립적인 인가 레이어를 동시에 관리해야 한다"는 운영상의 문제에서 온다.
...
두 인증 체계를 연결하려면 "IAM identity ↔ K8s identity" 매핑을 어딘가에서 관리해야 한다. 이 매핑이 틀리거나 누락되면 접근이 안 되고, 잘못 설정되면 과도한 권한이 부여된다. 초기 EKS에서는 `aws-auth` ConfigMap이 이 역할을 했는데, ConfigMap 하나가 잘못되면 클러스터 접근이 모두 막히는 단일 장애점(SPOF) 문제가 있었다. EKS Access Entry(2023년 말 도입)는 AWS API로 매핑을 관리하여 lockout 문제를 해결했지만, 매핑 자체가 필요하다는 근본적인 번거로움은 여전하다.

<br>

# 세 가지 인증 흐름 상세

앞서 정리한 세 가지 흐름을 하나씩 살펴본다. 풀어야 할 세 가지 문제(신뢰 수립 → 신분 매핑 → 최소 권한)가 실제로 발생하는 지점은 흐름 1과 흐름 3이고, 흐름 2는 그 문제가 없는 대조군이다.
...
![흐름 1: 사용자(IAM) → K8S API 인증/인가 흐름]({{site.url}}/assets/images/aws-authn-authz-overview.png){: .align-center}
...
![흐름 3: Pod(SA) → AWS API 인증/인가 흐름 (IRSA)]({{site.url}}/assets/images/aws-auth-pod-to-aws-overview.png){: .align-center}
...
| 흐름 | 인증(누구인가) | 인가(뭘 할 수 있나) | 복잡도 |
| --- | --- | --- | --- |
| **1. 사람 → K8S API** | IAM → STS pre-signed URL → K8S UserInfo | K8S RBAC (Role/RoleBinding) | **높음** — 두 세계를 넘어감 |
| **2. Pod → K8S API** | SA 토큰 → K8S API 직접 검증 | K8S RBAC (Role/RoleBinding) | **낮음** — K8S 안에서만 |
| **3. Pod → AWS API** | SA 토큰 → OIDC → STS AssumeRole | IAM Policy (Role에 부여) | **높음** — 두 세계를 넘어감 |

**EKS 인증/인가의 핵심 = "독립적으로 설계된 두 인증/인가 체계를, 보안을 유지하면서 양방향으로 연결하는 것"**

근본 문제(인증 주체의 불일치 + 인가 모델의 미스매치)에서 출발하면, 그 문제가 발생하는 지점이 곧 흐름 1과 3이고, 흐름 2는 그 문제가 없는 대조군이다.

## 기술 매핑

| 기술 | 어떤 문제를 푸는가 | 인증/인가 |
| --- | --- | --- |
| `aws-iam-authenticator` | 흐름 1의 신뢰 수립 | 인증 |
| `Access Entry` / `aws-auth` | 흐름 1의 신분 매핑 | 인증 → 인가 브릿지 |
| `Access Policy` / `K8S RBAC` | 흐름 1의 최소 권한 | 인가 |
| `OIDC Provider` | 흐름 3의 신뢰 수립 | 인증 |
| `IRSA 어노테이션` / `Pod Identity Association` | 흐름 3의 신분 매핑 | 인증 → 인가 브릿지 |
| `IAM Policy` (Role에 부여) | 흐름 3의 최소 권한 | 인가 |

## 앞으로 볼 내용

| 흐름 | 비중 | 학습 방향 |
| --- | --- | --- |
| **1. 사람 → K8S API** | 높음 | STS pre-signed URL + aws-iam-authenticator로 **인증**한 뒤, Access Entry/RBAC으로 **인가**까지 이어지는 전체 흐름 확인 |
| **2. Pod → K8S API** | 낮음 | SA/RBAC 기초 실습으로 K8S 단일 세계 안의 인증/인가 감을 잡는 수준 |
| **3. Pod → AWS API** | 높음 | OIDC + AssumeRoleWithWebIdentity로 **인증**한 뒤, IAM Policy로 **인가**까지 이어지는 전체 흐름 확인 |

핵심 학습 목표는 1번과 3번 흐름을 이해하는 것이다. 두 흐름 모두 서로 다른 인증 체계 사이에 신뢰를 수립하고(인증), 매핑된 주체에 적절한 권한을 부여하는(인가) 메커니즘이 필요하며, 그 메커니즘이 정반대 방향으로 동작한다. 2번은 IAM이 관여하지 않아 단순하므로, 기초 실습으로 SA/RBAC 감을 잡은 뒤 본격적인 학습은 1번과 3번에 집중한다.
