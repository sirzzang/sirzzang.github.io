---
title: "[LLM] LLM 서빙과 최적화 - 2.1. LLM과 트랜스포머 개요"
excerpt: "LLM 서빙을 이해하기 위한 언어 모델의 발전, 트랜스포머 아키텍처 개요에 대해 알아 보자."
categories:
  - AI
toc: true
header:
...
여기서 "병렬 처리가 가능해졌다"는 말의 축을 정확히 해 두자. 트랜스포머가 병렬화한 것은 **토큰(시퀀스) 축**이다. 한 층 안에서는 시퀀스의 모든 토큰이 동시에 계산된다. 셀프 어텐션은 시퀀스 전체를 행렬로 묶어 한 번에 곱하는 연산이고, MLP는 토큰마다 독립적으로 적용되기 때문이다. 반면 **깊이 축(블록 쌓기)은 여전히 순차다**. 앞 블록의 출력이 다음 블록의 입력이므로 — [시리즈 1편]({% post_url 2026-08-05-AI-LLM-Optimization-01-AI-Overview %})에서 본 합성 함수 구조 그대로 — 블록들은 차례로 통과할 수밖에 없다. 이 순차성은 뒤의 서빙 절에서 Pipeline Parallelism이 레이어 경계에서 잘리는 이유이기도 하다.

그런데도 GPU 활용에 문제가 없는 이유는, 순차 스텝의 **개수**와 스텝당 **일의 크기**가 RNN과 정반대이기 때문이다. RNN은 순차 스텝 수가 시퀀스 길이(수백~수천)이고 스텝마다 하는 일이 작아서 GPU 코어가 놀았다. 트랜스포머는 순차 스텝 수가 블록 수(GPT-2 small 기준 12)뿐이고, 각 스텝이 시퀀스 전체를 한꺼번에 처리하는 거대한 행렬 곱이라 스텝마다 GPU가 가득 찬다. 다만 이 병렬성은 입력을 한꺼번에 넣을 수 있을 때의 이야기다. 생성 단계에서는 토큰을 하나씩 만들어야 해서 토큰 축이 다시 순차가 되는데, 이 이야기가 아래 서빙 절의 prefill/decode 비대칭으로 이어진다.
...
GPT는 원본 트랜스포머 구조에서 **디코더 부분만** 사용하며, 텍스트 생성이 필요한 작업을 위해 고안되었다. 한 번에 한 토큰씩 예측해 텍스트를 생성하고, 이전 출력을 다시 입력에 붙여 다음을 예측하기 때문에 **자기회귀 모델(autoregressive model)**의 한 유형이다. 이 반복이 무한히 이어지는 것은 아니다. 모델이 **EOS(end-of-sequence)** 같은 특별 토큰을 다음 토큰으로 예측하거나 최대 길이에 닿으면 생성이 멈추는데, 종료 신호조차 별도의 장치가 아니라 여느 토큰과 똑같이 "다음 토큰 예측"으로 나온다는 점이 자기회귀 구조답다.
...
양자화, MoE, flash-attention은 각각 모델의 다른 부위를 겨냥한다. 파라미터가 어디에 몰려 있는지 알아야 어떤 기법이 이 모델에 유효한지 판단할 수 있는데, 그 분포 역시 아키텍처가 정한다. 트랜스포머 블록 하나에서 attention의 파라미터는 Q·K·V·출력 projection 네 개로 약 $4d^2$($d$는 모델 차원), MLP는 $d \to 4d$(up projection)와 $4d \to d$(down projection)의 큰 선형층 두 개로 약 $8d^2$이다. 즉 **블록 파라미터의 약 2/3가 MLP에 몰려 있다**. 그래서 파라미터 용량을 줄이거나 조건부로 활성화하는 양자화·MoE는 주로 MLP를 겨냥하고, flash-attention은 파라미터가 아니라 attention 연산의 메모리 접근 패턴을 겨냥한다.
