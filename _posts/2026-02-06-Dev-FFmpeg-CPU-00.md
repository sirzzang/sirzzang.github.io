...
다음 글부터 이 문제를 이해하기 위한 배경지식을 정리한다. CPU와 리눅스 스케줄러, cgroup, 컨테이너와 쿠버네티스의 CPU 리소스 관리, 그리고 ffmpeg의 스레드 모델이다. 배경지식을 바탕으로 가설을 세우고, 실험으로 검증해 나갈 것이다.

<br>

# 참고: CPU limit 1 core에서 goroutine 병렬 처리

위 코드에서 `errgroup`으로 goroutine 3개를 동시에 실행하고 있는데, CPU limit이 1 core라면 이 3개가 실제로 병렬로 동작하는 걸까?

결론부터 말하면, **동시에 "진행"은 되지만 물리적으로 "병렬 실행"은 안 된다.**

- **동시성(concurrency)은 유지된다**: Go 런타임 스케줄러가 goroutine을 번갈아 실행한다. 한 goroutine이 I/O 대기(예: 오브젝트 스토리지 업로드)에 들어가면 다른 goroutine이 CPU를 받아 실행된다.
- **병렬성(parallelism)은 사실상 없다**: 물리적으로 동시에 실행되려면 여러 코어가 필요한데, 1 core 분량의 시간만 있으니 한 시점에 하나만 실행된다.

하지만 이 문제에서 goroutine 수준의 동시성은 핵심이 아니다. 실제로 CPU를 잡아먹는 건 각 goroutine이 spawn하는 **ffmpeg 프로세스**다.

```
Goroutine 1 → ffmpeg 프로세스 (프레임 추출) → ~20개 스레드
Goroutine 2 → ffmpeg 프로세스 (썸네일 추출) → ~20개 스레드
Goroutine 3 → 업로드 (I/O 위주, CPU 거의 안 씀)
```

ffmpeg 프로세스 2개가 각각 호스트 코어 수(20) 기반으로 스레드를 만들면, **~40개 스레드가 1 core quota를 두고 경쟁**하게 된다. 업로드 goroutine은 네트워크 I/O 위주라 CPU를 거의 쓰지 않으므로 문제가 되지 않는다.

즉, `errgroup`의 병렬 구조 자체가 문제를 만드는 것이 아니라, 그 안에서 호출되는 **ffmpeg이 호스트 코어 수 기반으로 과도한 스레드를 생성하는 것**이 핵심이다. 이 부분은 [배경지식: CPU와 스케줄링]({% post_url 2026-02-06-Dev-FFmpeg-CPU-01 %})과 [배경지식: 영상 처리와 FFmpeg]({% post_url 2026-02-06-Dev-FFmpeg-CPU-03 %})에서 자세히 다룬다.
