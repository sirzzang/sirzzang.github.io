...
CPU가 유력해 보이는 이유는 있다. 나중에 살펴보겠지만, TTFB(Time To First Byte)는 두 환경 모두 약 0.2초로 유사했고, 차이가 벌어지는 구간이 ffmpeg 처리 구간이었기 때문이다. 다만, TTFB는 첫 바이트 수신까지의 시간만 측정하므로, 이것으로 배제할 수 있는 것은 **초기 연결 수준의 네트워크 오버헤드**뿐이다. ffmpeg 처리 중 발생하는 디스크 I/O(프레임 이미지 쓰기 등)나 처리 중간의 네트워크 지연까지 배제하려면 별도의 근거가 필요하다. 이 부분은 이후 실험에서 `time` 명령과 `cpu.stat`의 I/O wait 지표로 확인한다.

그런데 37배라는 차이가 단순히 "CPU 1 core 제한" 만으로 설명이 될까? 노드에 CPU가 충분히 있다 해도, 1 core면 1 core 속도로 돌아가야지 왜 그보다도 훨씬 느린 걸까?
...
이 시리즈에서는 "CPU throttling이 성능 저하의 주요 원인"이라는 가설을 세우고 실험으로 검증한다. 가설이 맞다면:

- `cpu.stat`에서 `nr_throttled / nr_periods` 비율이 높을 것이다
- `-threads` 옵션으로 ffmpeg 스레드 수를 제한하면 throttling이 감소하고 성능이 개선될 것이다
- `time` 명령 결과에서 `real`이 `user + sys`보다 현저히 클 것이다 (강제 대기 시간이 포함되므로)

반대로, 가설이 틀리다면:

- `-threads 1`로 스레드를 제한해도 성능 차이가 여전히 클 것이다 (스레드가 아닌 다른 원인)
- `time` 결과에서 `sys`가 높다면 I/O 병목일 수 있다
- `nr_throttled` 비율이 낮다면 CPU 제한이 아닌 다른 요인(메모리 압박, I/O 대기 등)을 의심해야 한다

<br>

# 시리즈 목차

다음 글부터 이 문제를 이해하기 위한 배경지식을 정리하고, 가설을 세운 뒤 실험으로 검증해 나간다.

1. [배경지식: CPU와 스케줄링]({% post_url 2026-02-06-Dev-FFmpeg-CPU-01 %})
2. [배경지식: cgroup, 컨테이너, 쿠버네티스]({% post_url 2026-02-06-Dev-FFmpeg-CPU-02 %})
3. [배경지식: 영상 처리와 FFmpeg]({% post_url 2026-02-06-Dev-FFmpeg-CPU-03 %})
4. 문제 원인 파악 및 튜닝
   - [4.0. 실험]({% post_url 2026-02-06-Dev-FFmpeg-CPU-04-00 %})
   - [4.1. 현상 관찰]({% post_url 2026-02-06-Dev-FFmpeg-CPU-04-01 %})
   - [4.2. 튜닝 실험 설계]({% post_url 2026-02-06-Dev-FFmpeg-CPU-04-02 %})
   - [4.3. 튜닝 실험: CPU Limit 관찰]({% post_url 2026-02-06-Dev-FFmpeg-CPU-04-03 %})
   - [4.4. 튜닝 실험: FFmpeg Thread 수 관찰]({% post_url 2026-02-06-Dev-FFmpeg-CPU-04-04 %})
5. [결론: CPU Limit을 설정하지 않기로 한 이유]({% post_url 2026-02-06-Dev-FFmpeg-CPU-05 %})
