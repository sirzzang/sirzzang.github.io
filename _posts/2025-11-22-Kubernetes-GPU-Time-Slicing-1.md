...
GPU를 분할하여 사용하는 방법에는 MIG, Time Slicing, MPS 등이 있다고 했는데, 여기서는 격리 수준이 가장 극명하게 대비되는 **MIG와 Time Slicing**을 비교한다.
...
둘 사이의 가장 큰 차이점은 Fault Isolation이 제공되는가이다. 참고로 MPS는 여러 프로세스가 하나의 CUDA Context를 공유하므로 Fault Isolation이 Time Slicing보다 오히려 약하다. 전체 메커니즘의 격리 수준 비교는 [GPU 공유 메커니즘: 개요]({% post_url 2025-11-22-Dev-GPU-Sharing-Mechanisms %}#비교) 글의 비교 표를 참고하자.
