...
> 이름에 "proxy"가 들어간다고 포워드 프록시(클라이언트 대신 서버에 요청하는 구조) 전용이 아니다. 클라이언트 요청을 받아서 백엔드로 "대신 전달(proxy)"하는 역할을 할 때 사용하는 헤더라는 의미다.

리버스 프록시를 활용해 Same-Origin을 구성하면 CORS 문제를 근본적으로 회피할 수 있다. SOP와 CORS의 동작 원리는 [Origin에 대한 고찰 - 정의, SOP, CORS]({% post_url 2025-05-10-CS-Origin-SOP-CORS %}#프록시로-same-origin-만들기)에 정리했다.
